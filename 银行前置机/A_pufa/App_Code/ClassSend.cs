...
public class ClassSend
{
	...
	public ClassSend()
	{
        ...
        //Dip = "10.112.26.73";
       // Ddk = "40012";
        Dip = ConfigurationManager.AppSettings["IP"].ToString();
        Ddk = ConfigurationManager.AppSettings["Port"].ToString();
    ...
    public Hashtable Get_ByteAndString_From_DataTable(DataTable dt, bool IsJM, string JMstr)
    {
        ...
        {
            //说明这个包是请求包，需要替换掉流水号字段
            string lsh = BankServiceHelper.GetLSH();//要替换成这个流水号
            if (dt.Columns.Contains("ExSerial") && dt.Rows[0]["ExSerial"] != null)
                dt.Rows[0]["ExSerial"] = (lsh == null ? dt.Rows[0]["ExSerial"] : lsh);
        ...
        //处理包头
        string tou = Dbaotou;
        tou = tou.Replace("加密标志三位", "110").Replace("MAC值八位", "");//切换成110加密方式，包头中MAC值和包头分开
        //tou = tou.Replace("加密标志三位", "000").Replace("MAC值八位", "00000000");   v1.0
        //处理包体
