...
{ 
	...
	{
		//formData.SOAOfficeCtrl.InsertSealFromURL( "images/sign02.esf");//不通过选择，直接插入指定签名
		...
		{
			formData.SOAOfficeCtrl.InsertSealFromURL("images/" + mObject.SelectValue);
		}
	}
}
//全文手写批注
...
{ 
	if (!bDocOpen)
		...
		formData.SOAOfficeCtrl.StartHandDraw();
} 
//分层显示全文手写批注
...
{ 
	if (!bDocOpen)
		...
		formData.SOAOfficeCtrl.ShowHandDrawDispBar();
} 
//给文档添加数字签名
...
{ 
	if (!bDocOpen)
		...
		//formData.SOAOfficeCtrl.AddDigitalSignature();
		alert("标准版无数字签名功能。");
} 

//验证数字签名
function soaValidateDigitalSignatures()
{ 
	if (!bDocOpen)
		alert("当前没有已打开的文档。");
	else		
		//formData.SOAOfficeCtrl.VerifyDigitalSignatures();
		alert("标准版无数字签名功能。");
} 
...
{ 
	...
	{
		//formData.SOAOfficeCtrl.InsertSealFromURL("images/seal02.esf");//不通过选择，直接插入指定印章
...
{ 
	if (!bDocOpen)
		...
		formData.SOAOfficeCtrl.InsertWebImage( "images/SOA_05.gif");
}
//页面设置
...
{ 
	...
	else
	{
		...
		// 刷新一下工具条
		if(formData.SOAOfficeCtrl.Toolbars)
		{
			formData.SOAOfficeCtrl.Toolbars = false;
			formData.SOAOfficeCtrl.Toolbars = true;
		}
	}
...
{ 
	...
	else
	{
		...
		if (mObject.SelectValue!="")
		{
			formData.SOAOfficeCtrl.ApplyTemplateFromURL("doc/" + mObject.SelectValue);
		}
	}
} 
//禁止/允许WORD鼠标拖曳功能
...
//显示保存失败信息
function soaDisplayError() 
{
	var  strErro;
	strErro = formData.SOAOfficeCtrl.LastErro;
	if  (strErro != ""){
...
{ 
	if (!bDocOpen){
		...
		formData.SOAOfficeCtrl.Document.ActiveWindow.Thumbnails = true;
	}		
...
{ 
	if (!bDocOpen){
		...
		formData.SOAOfficeCtrl.Document.ActiveWindow.Thumbnails = false;
	}		
