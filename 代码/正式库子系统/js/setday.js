<!--
document.writeln('<iframe  style="position: absolute; z-index: -1; width: expression(this.nextSibling.offsetWidth);height: expression(this.nextSibling.offsetHeight); top: expression(this.nextSibling.offsetTop);left: expression(this.nextSibling.offsetLeft);" frameborder="0"></iframe>');
...
document.writeln('             Author=meizz align=center><input Author=meizz type=button value="清空" onclick="meizzSetDay_reset()" ');
...
function setday(tt,obj) //主调函数
{
  ...
  var realtop=(ttyp=="image")? ttop+thei : ttop+thei+6;
  realtop = (realtop > 225) ? realtop - 225 : realtop;  //如果top大于225，则将日历控件的位置上移225px storm修改
  dads.top=realtop;
  dads.left = tleft;
...
function meizzSetDay_reset()
{
outObject.value="";  //将显示框的内容全部清空
}
...
function meizzDayClick(n)  //点击显示框选取日期，主输入函数*************
{
  ...
  if (outObject)
  {
    ...
    var float00 = true;
    if(typeof(ff_sptime) !=  'undefined' && ff_sptime == "DHSJ")
    {
    
    if(typeof(ff_sptime_b) !=  'undefined' && ff_sptime_b == "yes")
    {
       if( (typeof(ff_sptime_b_1) !=  'undefined' && outObject.name ==  ff_sptime_b_1)   ||  (typeof(ff_sptime_b_2) !=  'undefined' && outObject.name ==  ff_sptime_b_2) || (typeof(ff_sptime_b_3) !=  'undefined' && outObject.name ==  ff_sptime_b_3) || (typeof(ff_sptime_b_4) !=  'undefined' && outObject.name ==  ff_sptime_b_4))
       {float00 = false;}
    {
    ...
    var dq_mm = currdate.getMonth() + 1;
    if (dq_mm < 10){dq_mm = "0" + dq_mm;}
    var dq_dd = currdate.getDate();
    if (dq_dd < 10){dq_dd = "0" + dq_dd;}
    
    
    if(outObject.value<(currdate.getFullYear() + "-" + dq_mm + "-" + dq_dd))
    {
      
    //alert("要求到货日期不能小于当前日期");
      if(float00)
      {
      //alert("该日期不能小于当前日期");//2009-5-31修改
      }
    }
    }
    //========================================================================================================
    if(typeof(ff_sptime_wyh)!='undefined'&&ff_sptime_wyh == 'yes'&&typeof(document.getElementById('YQDDSJ'))!='undefined'&&document.getElementById('YQDDSJ').value != '' && typeof(document.getElementById('THSJ')) != 'undefined' && document.getElementById('THSJ').value != '' )
    {
       
       var YQDDSJ  = document.getElementById('YQDDSJ').value;
              var THSJ =document.getElementById('THSJ').value ;
       if(YQDDSJ > THSJ) 
        { 
            alert('要求到达时间必须小于提货时间！'); 
        } 
    ...
    //=================================================移动设备申请单 时间验证
       ...
       if(typeof(ff_sptime_moblie)!='undefined'&&ff_sptime_moblie == 'yes'&&typeof(document.getElementById('BJBSYSQDSYQSSJ'))!='undefined'&&document.getElementById('BJBSYSQDSYQSSJ').value != '' && typeof(document.getElementById('BJBSYSQDSYJZSJ')) != 'undefined' && document.getElementById('BJBSYSQDSYJZSJ').value != '' )
    {
       
       var YQDDSJ  = document.getElementById('BJBSYSQDSYQSSJ').value;
       var THSJ =document.getElementById('BJBSYSQDSYJZSJ').value ;
             
       if(YQDDSJ > THSJ) 
        { 
            alert('使用起始时间必须小于使用截止时间！'); 
        } 
...
function jumpNextControl(outObject)
{
var objName;
outObject.focus();
objName = outObject.name;
//frm = document.getElementById('frm1');
frm = document.forms[0];
for(var i=0;i<frm.elements.length;i++)
{
	if(objName == frm.elements[i].name)
	for(var j=i+1;j<frm.elements.length;j++)
	{
		if(frm.elements[j].type != "button" && frm.elements[j].type !="File" && frm.elements[j].type !="hidden")		{
			frm.elements[j].focus();return false;
		}
	}
}
}
...
//替换子表中的特殊字符
String.prototype.replaceAllts = stringReplaceAll;
function stringReplaceAll() {
//    return this.replace(/,/g, "，").replace(/\\/g, "").replace("<", "").replace(">", "").replace(/\'/g, "").replace(/\"/g, "").replace(/\//g, "a");
 return this.replace(/,/g, "，").replace(/\\/g, "").replace("<", "").replace(">", "").replace(/\'/g, "").replace(/\"/g, "");
