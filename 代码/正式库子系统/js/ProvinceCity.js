provinceArray = new Array("请选择省份", "北京市", "上海市", "天津市", "重庆市", "河北省", "山西省", "内蒙古自治区", "辽宁省", "吉林省", "黑龙江省", "江苏省", "浙江省", "安徽省", "福建省", "江西省", "山东省", "河南省", "湖北省", "湖南省", "广东省", "广西壮族自治区", "海南省", "四川省", "贵州省", "云南省", "西藏自治区", "陕西省", "甘肃省", "宁夏回族自治区", "青海省", "新疆维吾尔族自治区", "香港特别行政区", "澳门特别行政区", "台湾省", "其它");
...
function initProvince(IsPostBack,province,city) {
          
    
    document.all.selProvince.length = 0;
    for (i = 0; i < provinceArray.length; i++) {
        if (provinceArray[i] == "请选择省份") {
            document.all.selProvince.options[i] = new Option(provinceArray[i], "");
    ...
    document.all.selCity.options[document.all.selCity.length] = new Option("请选择城市", "");
    cityArray.unshift(new Array("请选择省份", "请选择城市"))


    //document.getElementById("selProvince").text = province;
   
    if (IsPostBack == 'True') {

        for (var i = 0; i < document.all.selProvince.options.length; i++)
        {
            if (document.all.selProvince.options[i].value == province) {
                document.all.selProvince.options[i].selected = true;
                getCity(province)
                document.all.selCity.value = city;
            }
        }       
           
        //document.getElementById("selProvince").value = province;
