...
public class KGclassYH
{
    ...
    {
        //获取所有空鼓入库的信息（k101）;
        string sql = "select ltrim(rtrim(INVTB.UDF01)) as 客户编号, ltrim(rtrim(INVTB.TB029)) as 空鼓类型, (case ltrim(rtrim(INVTB.TB029)) when '原装' then DYCPGG else (case  when charindex('(',MB003)>0 then ltrim(rtrim(substring(MB003,0,charindex('(',MB003)))) else ltrim(rtrim(MB003)) end) end) as 空鼓规格,cast(TB007 as int) as 剩余总量 from INVTB left join INVTA on TB002=TA002 and TB001=TA001 left join  CMSME on TA004=ME001 left join COPMA on INVTB.UDF01=MA001 left join INVMB on ltrim(rtrim(MB001))=LTrim(RTrim(TB004)) left join KGCPDZB on KGPH=LTrim(RTrim(TB004))  where TA001='k101' and ME002 like '%办事处%' and ltrim(rtrim(INVTB.UDF01))='" + UserNumber + "' and TA006='Y' ";
    ...
    public DataTable GetTable2(string UserNumber)
    {
        ...
        string sql = "select ltrim(rtrim(INVTB.UDF01)) as 客户编号, ltrim(rtrim(INVTB.TB029)) as 空鼓类型, (case ltrim(rtrim(INVTB.TB029)) when '原装' then DYCPGG else (case  when charindex('(',MB003)>0 then ltrim(rtrim(substring(MB003,0,charindex('(',MB003)))) else ltrim(rtrim(MB003)) end) end) as 空鼓规格,-cast(TB007 as int) as 剩余总量,LTrim(RTrim(TB004)) as 空鼓品号 from INVTB left join INVTA on TB002=TA002 and TB001=TA001 left join  CMSME on TA004=ME001 left join COPMA on INVTB.UDF01=MA001 left join INVMB on ltrim(rtrim(MB001))=LTrim(RTrim(TB004)) left join KGCPDZB on KGPH=LTrim(RTrim(TB004))  where TA001='k102' and ME002 like '%办事处%' and ltrim(rtrim(INVTB.UDF01))='" + UserNumber + "' and TA006='Y' and INVTA.UDF01 not like '%用户直通车%'";
    ...
    public DataTable GetAllKGCanUseByUserNumber(string UserNumber, DataTable dtNow)
    {
        ...
        DataTable dt6 = GetTable6(UserNumber);//终端用户空鼓调整表
        //将循环提货的，搞成跟第一个表一样的格式。       
   ...
   /// <param name="msg">如果数量不足，提示信息由此字段返回</param>
    ...
    private DataTable GetdtRK_profit(string UserNumber, string FWZDBH)
    {
        ...
        string sql_rk = "select ltrim(rtrim(INVTB.UDF01)) as 客户编号, ltrim(rtrim(INVTB.TB029)) as 空鼓类型, (case ltrim(rtrim(INVTB.TB029)) when '原装' then DYCPGG else (case  when charindex('(',MB003)>0 then ltrim(rtrim(substring(MB003,0,charindex('(',MB003)))) else ltrim(rtrim(MB003)) end) end) as 空鼓规格,cast(TB007 as int) as 剩余总量 from INVTB left join INVTA on TB002=TA002 and TB001=TA001 left join  CMSME on TA004=ME001 left join COPMA on INVTB.UDF01=MA001 left join INVMB on ltrim(rtrim(MB001))=LTrim(RTrim(TB004)) left join KGCPDZB on KGPH=LTrim(RTrim(TB004))  where TA001='k101' and ME002 like '%办事处%' and ltrim(rtrim(INVTB.UDF01))='" + UserNumber + "' and TA006='Y' and ltrim(rtrim(TB012))='" + FWZDBH + "' and convert(datetime,TA003)>='2012-01-01 0:00:00'";

        string sql_ck = "select ltrim(rtrim(INVTB.UDF01)) as 客户编号, ltrim(rtrim(INVTB.TB029)) as 空鼓类型, (case ltrim(rtrim(INVTB.TB029)) when '原装' then DYCPGG else ltrim(rtrim(MB003)) end)  as 空鼓规格,-cast(TB007 as int) as 剩余总量 from INVTB left join INVTA on TB002=TA002 and TB001=TA001 left join  CMSME on TA004=ME001 left join COPMA on INVTB.UDF01=MA001 left join INVMB on ltrim(rtrim(MB001))=LTrim(RTrim(TB004)) left join KGCPDZB on KGPH=LTrim(RTrim(TB004))  where TA001='k102' and ME002 like '%办事处%' and ltrim(rtrim(INVTB.UDF01))='" + UserNumber + "' and TA006='Y' and ltrim(rtrim(TB012))='" + FWZDBH + "' and INVTA.UDF01 not like '%用户直通车%' and convert(datetime,TA003)>='2012-01-01 0:00:00'";
    ...
    public double GetCreditMoney(string UserNumber)
    {
        ...
        if (ds8 != null && ds8.Tables[0].Rows.Count > 0)
        {
            for (int i = 0; i < ds8.Tables[0].Rows.Count; i++)
            {
                ...
                string sqlXH = "select isnull(sum(cast(TH008 as int)),0) as 已交付数量 from COPTG left join COPTH on TG001=TH001 and TG002=TH002 where ltrim(rtrim(TG004))='" + UserNumber + "' and COPTG.UDF01='" + ddh + "' and TG001 in ('" + db + "') and (case substring(ltrim(rtrim(TH004)),1,3) when '151' then '绿装'  when '150' then '蓝装' end)='" + ds8.Tables[0].Rows[i]["产品类别"].ToString() + "' and ltrim(rtrim(substring(TH005,charindex(' ',TH005),len(TH005))))='" + ds8.Tables[0].Rows[i]["产品型号"].ToString() + "' and TG023='Y' and cast(TH012 as numeric(18,2))=" + cpjg + "";
