...
public class BankRoute
{
    ...
    public BankInit GetUserInformation(string umail)
    {
        ...
        string sql = "SELECT *, Rt=(CASE WHEN I_KHYH='浦发银行' THEN 'BankPufa' WHEN I_KHYH='平安银行' THEN 'BankPingan' ELSE '无法找到所属银行' END) FROM AAA_DLZHXXB WHERE B_DLYX='" + umail.Trim() + "' AND I_DSFCGZT='开通' AND S_SFYBJJRSHTG='是' AND S_SFYBFGSSHTG='是' AND B_SFYZYX='是' AND B_SFYXDL='是'";
