...
public partial class Web_pagerdemo_commonpagernew : System.Web.UI.UserControl
{
    ...
    public Hashtable HTwhere
    {
        ...
        get {
            Hashtable ht_whereHashTable = new Hashtable();
            ht_whereHashTable["page_index"] = ViewState["page_index"];
            ht_whereHashTable["page_size"] = ViewState["page_size"];
            return ht_whereHashTable;
    ...
    private void resetBFY()
    {
        ...
        if (HttpContext.Current.Request.Url.PathAndQuery.IndexOf("/YHZTC") > 0)
        {
            //Cpageshow.Attributes.Add("style", "display:none;");
        }
    ...
    public void GetFYdataAndRaiseEvent()
    {
        ...
        //更新文字描述信息
        tdpagecount.InnerHtml = "共" + ((Convert.ToInt32(ViewState["page_count"].ToString()) ==-1||(Convert.ToInt32(ViewState["page_count"].ToString())==0)) ? 0 : Convert.ToInt32(ViewState["page_count"].ToString())) + "页";
        int pageIndex = (Convert.ToInt32(ViewState["page_index"]) + 1);
        int pageCount = Convert.ToInt32(ViewState["page_count"].ToString());
        //当前页面起始记录号
        int currentPageIndexNumber = 0;
        //当期页面结束记录号
        int currentPageIndexEndNumber =0;
        if (pageIndex == 1)
        {
            pagebox_first.Attributes["class"] = "pagebox_firstUnable";
            linkFirst.Enabled = false;
            pagebox_last.Attributes["class"] = "pagebox_lastUnable";
            linkLast.Enabled = false;
            if (pageCount == 1 || pageCount == 0)
            {
                pagebox_next.Attributes["class"] = "pagebox_nextUnable";
                linkNext.Enabled = false;
                pagebox_end.Attributes["class"] = "pagebox_endUnable";
            ...
            {
                pagebox_next.Attributes["class"] = "pagebox_next";
                linkNext.Enabled = true;
                pagebox_end.Attributes["class"] = "pagebox_end";
                linkEnd.Enabled = true;
            }
            currentPageIndexNumber = Convert.ToInt32(ViewState["page_count"].ToString()) == -1 || Convert.ToInt32(ViewState["page_count"].ToString()) == 0 ? 0 : (pageIndex - 1) * (Convert.ToInt32(ViewState["page_size"].ToString())) + 1;
            if (Convert.ToInt32(ViewState["record_count"].ToString()) > (pageIndex * (Convert.ToInt32(ViewState["page_size"].ToString()))))
            {
                currentPageIndexEndNumber = pageIndex * (Convert.ToInt32(ViewState["page_size"].ToString()));
            }
            else
            {
                currentPageIndexEndNumber = Convert.ToInt32(ViewState["record_count"].ToString());
            }
        }
        else if (pageIndex > 1&&pageIndex<pageCount)
        {
            pagebox_first.Attributes["class"] = "pagebox_first";
            linkFirst.Enabled = true;
            pagebox_last.Attributes["class"] = "pagebox_last";
            linkLast.Enabled = true;
            pagebox_next.Attributes["class"] = "pagebox_next";
            linkNext.Enabled = true;
            pagebox_end.Attributes["class"] = "pagebox_end";
            linkEnd.Enabled = true;
            currentPageIndexNumber = (pageIndex - 1) * (Convert.ToInt32(ViewState["page_size"].ToString())) + 1;
            currentPageIndexEndNumber = pageIndex * (Convert.ToInt32(ViewState["page_size"].ToString()));
        }
        ...
        tdpagecountdesc.InnerHtml = "显示" + currentPageIndexNumber + "-" + currentPageIndexEndNumber + "，共" + ViewState["record_count"].ToString() + "条";
        //更新转到的显示
        tbgopage.Text = Convert.ToInt32(ViewState["page_count"].ToString()) == -1 || Convert.ToInt32(ViewState["page_count"].ToString()) ==0? "0" : (Convert.ToInt32(ViewState["page_index"]) + 1).ToString();
    ...
    public DataSet getDataSet_FY(string dg_once)
    {
        ...
        {
            if (ViewState["this_dblink"].ToString() == "ERP")
            {
                csstr = "fmConnection";
            }
            if (ViewState["this_dblink"].ToString() == "erpFMOPConn")
            {
                csstr = "erpFMOPConn";
            }
            if (ViewState["this_dblink"].ToString() == "fmConnection2")
            {
                csstr = "fmConnection2";
            }
            if (ViewState["this_dblink"].ToString() == "fmConnectionWrite")
            {
                csstr = "fmConnectionWrite";
            }
        }
        ...
        //ViewState["page_size"] = "5";
        //ViewState["serach_Row_str"] = " * ";
        //ViewState["search_tbname"] = " FWPT_tbMenuSort ";
        //ViewState["search_mainid"] = " ID ";
        //ViewState["search_str_where"] = " ID is not null ";
        //ViewState["search_paixu"] = " DESC ";
        //ViewState["search_paixuZD"] = " SortOrder ";
        //ViewState["record_count"] = "0";
        //ViewState["page_count"] = "0";
        ...
        //获取数据
        return_ht = I_DBL.RunProc_CMD(ViewState["GetCustomersDataPage_NAME"].ToString(), DataSet_Beuse, Hashtable_PutIn, ref Hashtable_PutOut);
        int page_count = 0;
