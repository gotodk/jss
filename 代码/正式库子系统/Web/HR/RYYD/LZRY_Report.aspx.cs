...
public partial class Web_HR_RYYD_LZRY_Report : System.Web.UI.Page
{
    ...
    protected void Button1_Click1(object sender, EventArgs e)
    {
        ...
        lbltitle.Text = kssj.Value.ToString() + "至" + jssj.Value.ToString() + "离职人员名单";
        string sql = "select (row_number() over (order by bm,lzsj)) as 序号, ygbh as 员工工号,xm as 员工姓名,BM as 部门,gwmc as 岗位,cast(month(lzsj) as varchar(2))+'月' as 离职月份,convert(varchar(10),lzsj,120) as 离职时间,(case when datalength(lzyy)<=0 then '无' else lzyy end ) as 备注 from RLZY_YGGX_LZGL where ls like '%" + ls + "%' and bm like '%" + bm + "%' and lzsj between '" + StartTime + "' and '" + EndTime + "' order by bm,lzsj";
    ...
    public void ToExcel(System.Web.UI.Control ctl)
    {
        ...
        HttpContext.Current.Response.AppendHeader("Content-Disposition", "attachment;filename=LiZhi.xls");
