...
public partial class Web_HR_RYYD_GQLDHT_Report : System.Web.UI.Page
{
    ...
    protected void Button1_Click1(object sender, EventArgs e)
    {
        ...
        lbltitle.Text = kssj.Value.ToString() + "至" + jssj.Value.ToString() + "改签劳动合同人员名单";

        string sql = "select (row_number() over (order by bm,htkssj)) as 序号, ygbh as 员工工号,xm as 员工姓名,BM as 部门,gwmc as 岗位,cast(month(htkssj) as varchar(2))+'月' as 劳动合同签订月份,convert(varchar(10),htkssj,120) as 劳动合同开始时间,isnull(convert(varchar(10),syqkssj,120)+'到'+convert(varchar(10),syqjssj,120),'无') as 试用期,isnull(yygxs,'无') as 原用工形式, '' as 备注 from RLZY_YGGX_HTGL where ls like '%" + ls + "%' and bm like '%" + bm + "%' and htkssj between '" + StartTime + "' and '" + EndTime + "' order by bm,htkssj";
    ...
    public void ToExcel(System.Web.UI.Control ctl)
    {
        ...
        HttpContext.Current.Response.AppendHeader("Content-Disposition", "attachment;filename=GaiQian.xls");
