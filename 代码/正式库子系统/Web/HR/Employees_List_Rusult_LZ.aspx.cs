...
public partial class Web_HR_Employees_List_Rusult_LZ : System.Web.UI.Page
{
    ...
    {
        znds = DbHelperSQL.Query("select EmployeeZN_Sex,EmployeeZN_BirthDay from HR_Employees_ZNZK where parentNumber ='" + prNunber + "'");
    }
    /// <summary>
    /// 取得任职资格证书名称
    /// </summary>
    ...
    {
        string sql = "select * from HR_Employees_RZZGZS where parentNumber ='" + prNunber + "'";
    ...
    {
        string sql = " SELECT  XM, YGW, ZW, DGSJ, TGLB,BZ FROM  RLZY_YGGX_TGGL WHERE YGBH='" + HYBH + "'";
    ...
    {
        Response.Redirect("Employee_List_LZ.aspx");
    }
}
