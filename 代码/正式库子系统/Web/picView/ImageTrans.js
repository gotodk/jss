...
ImageTrans.prototype = {
    ...
    reset: function () {
        if (this._support) {
            ...
            //设置图片显示的大小。根据外层的div的宽、高来确定内层图片显示的比例。
            var img_width = this._img.clientWidth;
            var img_height = this._img.clientHeight;
            if (img_width <= this._clientWidth && img_height <= this._clientHeight)
            { this._x = this._y = 1; }
            if (img_width < this._clientWidth && img_height > this._clientHeight) {
                var ytemp = 1 - (img_height - this._clientHeight) / img_height;
                this._x = this._y = ytemp.toFixed(2);
            }
            if (img_width > this._clientWidth && img_height < this._clientHeight) {
                var xtemp = 1 - (img_width - this._clientWidth) / img_width;
                this._x = this._y = xtemp.toFixed(2);

            }
            if (img_width > this._clientWidth && img_height > this._clientHeight) {
                var xtemp = 1 - (img_width - this._clientWidth) / img_width;
                var ytemp = 1 - (img_height - this._clientHeight) / img_height;
                this._x = this._y = xtemp < ytemp ? xtemp.toFixed(2) : ytemp.toFixed(2);
            }
            //            this._x = this._y = 1; 
            this._radian = 0;
            this._show();
