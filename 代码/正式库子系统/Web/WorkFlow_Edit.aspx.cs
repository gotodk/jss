...
public partial class WorkFlow_Edit : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
            if (Request["module"] != null)
            {
                ...
                {
                    Response.Clear();
                    Response.Write("<script language=javascript>window.alert('没有搜索到此项内容！');history.back();</script>");
                    Response.End();
                    return;
                }
                else
    ...
    {
		if (Request["pagenumber"] != null && Request["pagenumber"] != string.Empty)
		{
			pagenumber = Convert.ToInt32(Request["pagenumber"]);
		}
    ...
    protected void initpage()
    {
		...
		modifytype = this.ViewState["modifytype"].ToString();
    }
    ...
    protected int xmlParse()
    {
        ...
        if (xlist != null)
        {
            fieldName = new string[xlist.Count + 6];
    ...
    protected DataSet addParamters()
    {
        ...
        Hesion.Brick.Core.FMEvengLog.SaveToLog(User.Identity.Name, module, Request.UserHostAddress, "WorkFlow_Edit.aspx", "", sqlstr, dailiren);
        return dsNew;
    }
    ...
    protected void addTable()
    {
        ...
        sb.Append(htmlWrite.addViewTable("test", addParamters(), caption, 2));
	...
	protected void addSQL()
	{
		...
		if (xlist != null)
		{
			...
			foreach (XmlNode item in xlist)
			{
				...
				else
				{
					...
					{
                        //string test = "txt" + XMLHelper.GetSingleString(item, "name", "");
        ...
        //else
        //{
        //    strNumber = ("  number like '%" + Request["txtNum"].ToString() + "%'");
    ...
    protected void addrole()
    {
        ...
        if (!wfr.CanModify)
        {
			...
			{
				modifytype = "1";
				this.ViewState["modifytype"] = modifytype;
				return;
			}
            if (!wfr.CanAdd)
            {
                if (!wfr.CanView)
                {
                    if (!wf.check.isCheckName (id))//FMOP.CI.CheckInfo.canCheck(module, id)
                    {
                        Response.Clear();
        ...
        {
			modifytype = wfr.ModifyType.ToString();
			if (modifytype == null || modifytype == string.Empty)
			{
				modifytype = "1";
			}
			this.ViewState["modifytype"] = modifytype;
