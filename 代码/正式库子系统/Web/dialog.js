...
var IMGFOLDERPATH = '/web/dialog_img/'; //图片路径配置
...
Dialog.prototype.show = function() {
    ...
    arr.push("            </div></td></tr>");
    arr.push("      </table><a href='#;' onfocus='$1(\"_forTab_" + this.ID + "\").focus();'></a></td>");
    ...
    var div = pw.$1("_DialogDiv_" + this.ID);
    if (!div) {
    ...
    if (this.innerElementId) {
        var innerElement = $1(this.innerElementId);
        ...
        if (isIE) {
            ...
            pw.$1("_Covering_" + this.ID).parentNode.appendChild(fragment)
        } else {
            pw.$1("_Covering_" + this.ID).parentNode.appendChild(innerElement)
        }
    }
    pw.$1("_DialogDiv_" + this.ID).DialogInstance = this;
    if (this.URL)
        pw.$1("_DialogFrame_" + this.ID).DialogInstance = this;
    pw.Drag.init(pw.$1("_draghandle_" + this.ID), pw.$1("_DialogDiv_" + this.ID)); //注册拖拽方法
    if (!isIE) {
        pw.$1("_DialogDiv_" + this.ID).dialogId = this.ID;
        pw.$1("_DialogDiv_" + this.ID).onDragStart = function() { $1("_Covering_" + this.dialogId).style.display = "" }
        pw.$1("_DialogDiv_" + this.ID).onDragEnd = function() { $1("_Covering_" + this.dialogId).style.display = "none" }
    }

    this.OKButton = pw.$1("_ButtonOK_" + this.ID);
    this.CancelButton = pw.$1("_ButtonCancel_" + this.ID);

    //显示标题图片
    if (this.ShowMessageRow) {
        $E.show(pw.$1("_MessageRow_" + this.ID));
        if (this.MessageTitle) {
            pw.$1("_MessageTitle_" + this.ID).innerHTML = this.MessageTitle;
        }
        if (this.Message) {
            pw.$1("_Message_" + this.ID).innerHTML = this.Message;
        }
    }

    //显示按钮栏
    if (!this.ShowButtonRow) {
        pw.$1("_ButtonRow_" + this.ID).hide();
    }
    if (this.CancelEvent) {
    ...
    if (!this.AlertFlag) {
        ...
        if (!bgdiv) {
            ...
            bgdiv.style.cssText = "background-color:#333;position:absolute;left:0px;top:0px;opacity:0.4;filter:alpha(opacity=40);width:100%;height:" + sh + "px;z-index:8991";
    ...
    this.DialogDiv.style.cssText = "position:absolute; display:block;z-index:" + (this.AlertFlag ? 8992 : 8990) + ";left:" + this.Left + "px;top:" + this.Top + "px";
    ...
    if (!this.AlertFlag) {
        ...
        while (win != win.parent) {//需要考虑父窗口是弹出窗口中的一个iframe的情况
            if (win._DialogInstance) {
                win._DialogInstance.DialogDiv.style.zIndex = 8959;
                flag = true;
                break;
            }
            win = win.parent;
        ...
        if (!flag) {
            bgdiv.style.cssText = "background-color:#333;position:absolute;left:0px;top:0px;opacity:0.4;filter:alpha(opacity=40);width:100%;height:" + sh + "px;z-index:8960";
        }
...
Dialog.prototype.close = function() {
    if (this.innerElementId) {
        var innerElement = $E.getTopLevelWindow().$1(this.innerElementId);
    ...
    if (this.WindowFlag) {
        ...
        while (win != win.parent) {
            if (win._DialogInstance) {
                flag = true;
                win._DialogInstance.DialogDiv.style.zIndex = 8960;
                break;
            }
            win = win.parent;
        }
        if (this.AlertFlag) {
            $E.hide(pw.$1("_AlertBGDiv"));
        }
        if (!flag && !this.AlertFlag) {//此处是为处理弹出窗口被关闭后iframe立即被重定向时背景层不消失的问题
            pw.eval('window._OpacityFunc = function(){var w = $E.getTopLevelWindow();$E.hide(w.$1("_DialogBGDiv"));}');
            pw._OpacityFunc();
        }
...
Dialog.prototype.addButton = function(id, txt, func) {
    ...
    var pw = $E.getTopLevelWindow();
    pw.$1("_DialogButtons_" + this.ID).$T("input")[0].getParent("a").insertAdjacentHTML("beforeBegin", html);
    pw.$1("_Button_" + this.ID + "_" + id).onclick = func;
}
...
Dialog.alert = function(msg, func, w, h) {
    ...
    diag.show();
    pw.$1("_AlertBGDiv").style.display = "";
    $E.hide(pw.$1("_ButtonOK_" + diag.ID));
    var win = pw.$1("_DialogFrame_" + diag.ID).contentWindow;
    ...
    pw.$1("_DialogButtons_" + diag.ID).style.textAlign = "center";
}
...
Dialog.confirm = function(msg, func1, func2, w, h) {
    ...
    diag.show();
    pw.$1("_AlertBGDiv").style.dispaly = "";
    var win = pw.$1("_DialogFrame_" + diag.ID).contentWindow;
    var doc = win.document;
    doc.open();
    doc.write("<body oncontextmenu='return false;'></body>");
...
//startAdd
//提醒
Dialog.PromptConfirm = function (msg, func1, func2, w, h) {
    var pw = $E.getTopLevelWindow()
    var diag = new Dialog("_DialogAlert" + Dialog.AlertNo++);
    diag.Width = w ? w : 300;
    diag.Height = h ? h : 120;
    ...
    var arr = [];
    arr.push("<table height='100%' border='0' align='center' cellpadding='10' cellspacing='0'>");
    arr.push("<tr><td align='right'><img id='Icon' src='" + IMGFOLDERPATH + "Prompt.gif' width='34' height='34' align='absmiddle'></td>");
    arr.push("<td align='left' id='Message' style='font-size:9pt'>" + msg + "</td></tr></table>");
    var div = doc.createElement("div");
    div.innerHTML = arr.join('');
    doc.body.appendChild(div);
...
//警告
Dialog.WarningConfirm = function (msg, func1, func2, w, h) {
    var pw = $E.getTopLevelWindow()
    var diag = new Dialog("_DialogAlert" + Dialog.AlertNo++);
    diag.Width = w ? w : 300;
    diag.Height = h ? h : 120;
...
//错误
Dialog.ErrorConfirm = function (msg, func1, func2, w, h) {
    var pw = $E.getTopLevelWindow()
    var diag = new Dialog("_DialogAlert" + Dialog.AlertNo++);
    diag.Width = w ? w : 300;
    diag.Height = h ? h : 120;
    ...
    var arr = [];
    arr.push("<table height='100%' border='0' align='center' cellpadding='10' cellspacing='0'>");
    arr.push("<tr><td align='right'><img id='Icon' src='" + IMGFOLDERPATH + "Error.gif' width='34' height='34' align='absmiddle'></td>");
    arr.push("<td align='left' id='Message' style='font-size:9pt'>" + msg + "</td></tr></table>");
    var div = doc.createElement("div");
    div.innerHTML = arr.join('');
    doc.body.appendChild(div);
...
//问号
Dialog.WenhaoConfirm = function (msg, func1, func2, w, h) {
    var pw = $E.getTopLevelWindow()
    var diag = new Dialog("_DialogAlert" + Dialog.AlertNo++);
    diag.Width = w ? w : 300;
    diag.Height = h ? h : 120;
    ...
    arr.push("<table height='100%' border='0' align='center' cellpadding='10' cellspacing='0'>");
    arr.push("<tr>");
    arr.push("<td align='left' id='Message' style='font-size:12pt'>" + msg + "</td></tr></table>");
    var div = doc.createElement("div");
    div.innerHTML = arr.join('');
    doc.body.appendChild(div);
    doc.close();
    ...
    if (d) {
        try {
            ...
            if (!this.AlertFlag) {
                ...
                if (!flag) {
                    $E.getTopLevelWindow().$1("_DialogBGDiv").style.opacity = "0.4";
                    $E.getTopLevelWindow().$1("_DialogBGDiv").style.filter = "alpha(opacity=40)";
                }
            }
            if (d.AlertFlag) {
                $E.show($E.getTopLevelWindow().$1("_AlertBGDiv"));
            }
...
Dialog.setPosition = function() {
    ...
    for (i = 0; i < DialogArr.length; i++) {
        pw.$1("_DialogDiv_" + DialogArr[i]).DialogInstance.setPosition();
    }
}
Dialog.prototype.setPosition = function() {
    ...
    pw.$1(this.bgdivID).style.height = sh + "px";
}
