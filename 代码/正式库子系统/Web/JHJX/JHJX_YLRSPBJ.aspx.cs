...
public partial class Web_JHJX_JHJX_YLRSPBJ : System.Web.UI.Page
{
    ...
    {
        di = DbHelperSQL.Query("SELECT [Number],[SPMC],[SPGG] ,[SSFLBH],[JJDW],[JJPL] ,[SPMS],[SCZZYQ],[FHWTGSJ],[FHWTGCZR],[QDSXSJ],[QDSXCZR],[SPZT],[CheckState],[CreateUser] ,[CreateTime],[EditTime],[Editer]  FROM [AAA_SPYLRXXB] WHERE Number='" + num + "'").Tables[0];
        ...
        yspgg = txtSPGG.Text = di.Rows[0]["SPGG"].ToString().Trim();
        ...
        ViewState["SPBH"] = di.Rows[0]["Number"].ToString().Trim();
    }
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            di.Rows[0]["SPGG"] = txtSPGG.Text.Trim();
            di.Rows[0]["SPMS"] = txtSPMS.Text.Trim();
            di.Rows[0]["JJPL"] = txtJJPL.Text.Trim();
            di.Rows[0]["SCZZYQ"] = zzxq.Trim();
            di.Rows[0]["SPZT"] = "未复核";
            di.Rows[0]["Editer"] =User.Identity.Name;
            di.Rows[0]["EditTime"] = DbHelperSQL.GetSingle("select getdate()").ToString();

            DataTable dc = di.GetChanges(DataRowState.Modified);

            SqlConnection connection = new SqlConnection(connectionString);
            SqlDataAdapter adapter = new SqlDataAdapter("SELECT [Number],[SPMC],[SPGG] ,[SSFLBH],[JJDW],[JJPL] ,[SPMS],[SCZZYQ],[SPZT],[CheckState],[Editer] ,[EditTime] FROM [AAA_SPYLRXXB]", connection);
            SqlCommandBuilder builder = new SqlCommandBuilder(adapter);

            if (adapter.Update(dc) > 0)
            {

                MessageBox.ShowAndRedirect(this, "数据更新成功！", "JHJX_YLRSPGL.aspx");
            }
            else
            {
