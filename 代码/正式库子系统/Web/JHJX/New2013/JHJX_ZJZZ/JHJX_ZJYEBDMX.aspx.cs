...
public partial class Web_JHJX_New2013_JHJX_JHJX_ZJZZ_JHJX_ZJYEBDMX : System.Web.UI.Page
{
    ...
    {
        DataSet ds = DbHelperSQL.Query("select XM, XZ as XZvalue,(case when LEN (XZ)>10 then SUBSTRING (xz,0,11)+'...' else XZ end) as XZtext from AAA_moneyDZB where 1=1 and  SJLX='实'");
        ViewState["XMXZ"] = ds;
        setXM(ds.Tables[0]);
        setXZ(ds.Tables[0]);       
    }
    ...
    private void MyWebControl_OnNeedLoadData(DataSet NewDS, string ERRinfo)
    {
        ...
        {
            //rptZJYEBDMX.DataSource = DbHelperSQL.Query("select a.Number,a.LSCSSJ as 变动时间,DDZH.I_JYFMC 交易方名称,DDZH.B_DLYX '交易方账号',DDZH.I_YWGLBMFL '业务管理部门分类',DDZH.I_PTGLJG '分公司名称',a.ZY as 摘要,a.XM as 项目,a.XZ as 性质,case when  a.YSLX in ('增加','解冻') then '+'+convert(varchar(20),convert(decimal(18,4),a.JE)) else '-'+convert(varchar(20),convert(decimal(18,4),a.JE)) end as 金额 from AAA_ZKLSMXB as a inner join AAA_moneyDZB as b on a.xm=b.xm and a.xz=b.xz  and a.sjlx=b.sjlx  and a.sjlx='实'  inner join AAA_DLZHXXB as DDZH on a.DLYX=DDZH.B_DLYX   where 1!=1");
    ...
    private Hashtable SetV()
    {
        ...
        //ht_where["search_tbname"] = " (select a.Number,a.LSCSSJ as 变动时间,DDZH.I_JYFMC 交易方名称,DDZH.B_DLYX '交易方账号',DDZH.I_YWGLBMFL '业务管理部门分类',DDZH.I_PTGLJG '分公司名称',a.ZY as 摘要,a.XM as 项目,a.XZ as 性质,case when  a.YSLX in ('增加','解冻') then '+'+convert(varchar(20),convert(decimal(18,2),a.JE)) else '-'+convert(varchar(20),convert(decimal(18,2),a.JE)) end as 金额 from AAA_ZKLSMXB as a inner join AAA_moneyDZB as b on a.xm=b.xm and a.xz=b.xz  and a.sjlx=b.sjlx  and a.sjlx='实'  inner join AAA_DLZHXXB as DDZH on a.DLYX=DDZH.B_DLYX) as tab";  //检索的表
        /*---shiyan 2013-12-26 数据获取优化---*/
        ht_where["GetCustomersDataPage_NAME"] = "GetCustomersDataPage";//这个可以不设置,默认是  
        ht_where["serach_Row_str"] = " a.Number,a.LSCSSJ as 变动时间,DDZH.I_JYFMC 交易方名称,a.DLYX as 交易方账号,DDZH.I_YWGLBMFL as 业务管理部门分类,DDZH.I_PTGLJG as 分公司名称,a.ZY as 摘要,a.XM as 项目,a.XZ as 性质,case when (a.YSLX='增加' or a.YSLX='解冻') then '+'+convert(varchar(20),convert(decimal(18,2),a.JE)) else '-'+convert(varchar(20),convert(decimal(18,2),a.JE)) end as 金额 ";
        ht_where["search_tbname"] = " AAA_ZKLSMXB as a inner join AAA_DLZHXXB as DDZH on a.DLYX=DDZH.B_DLYX ";  //检索的表
        /*---shiyan 结束---*/
        ht_where["search_mainid"] = " a.Number ";  //所检索表的主键
        ht_where["search_str_where"] = " a.sjlx='实' ";  //检索条件  
    ...
    protected void btnToExcel_Click(object sender, EventArgs e)
    {
        ...
        MXC.goxls(NewDS, "Report_" + DateTime.Now.ToString("yyyy-MM-dd:hh-mm-sss") + ".xls", "资金余额变动明细", "资金余额变动明细", 15);
    }
}