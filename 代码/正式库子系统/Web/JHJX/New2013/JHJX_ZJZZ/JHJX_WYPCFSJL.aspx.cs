...
public partial class Web_JHJX_New2013_JHJX_JHJX_ZJZZ_JHJX_WYPCFSJL : System.Web.UI.Page
{
    ...
    {
        DataSet ds = DbHelperSQL.Query("select XM, XZ as XZvalue,(case when LEN (XZ)>10 then SUBSTRING (xz,0,11)+'...' else XZ end) as XZtext from AAA_moneyDZB where 1=1  and dymkbh='7' and   sjlx='预' order by dymkbh");
        ViewState["XMXZ"] = ds;
        setXM(ds.Tables[0]);
    ...
    private Hashtable SetV()
    {
        ...
        ht_where["search_str_where"] = " a.sjlx='预' and isnull(b.dymkbh,'')='7' ";  //检索条件  
    ...
    protected void btnToExcel_Click(object sender, EventArgs e)
    {
        ...
        //string sql1 = "select * from (select a.Number,a.LSCSSJ as 变动时间,DDZH.I_JYFMC '交易方名称',DDZH.B_DLYX '交易方账号',DDZH.I_YWGLBMFL '业务管理部门分类',DDZH.I_PTGLJG '分公司名称',a.ZY as 摘要,a.XM as 项目,a.XZ as 性质,a.JE as 金额 from  AAA_ZKLSMXB as a inner join AAA_moneyDZB as b on a.xm=b.xm and a.xz=b.xz  and a.sjlx=b.sjlx  inner join AAA_DLZHXXB as DDZH on a.DLYX=DDZH.B_DLYX where 1=1 and a.sjlx='预' and isnull(b.dymkbh,'')='7') as tab where 1=1 " + sql_where + " order by 变动时间 DESC,Number desc";

        string sql1 = "select a.Number,a.LSCSSJ as 变动时间,DDZH.I_JYFMC as 交易方名称,DDZH.B_DLYX as 交易方账号,DDZH.I_YWGLBMFL as 业务管理部门分类,DDZH.I_PTGLJG as 分公司名称,a.ZY as 摘要,a.XM as 项目,a.XZ as 性质,a.JE as 金额 from  AAA_ZKLSMXB as a inner join AAA_moneyDZB as b on a.xm=b.xm and a.xz=b.xz  and a.sjlx=b.sjlx  inner join AAA_DLZHXXB as DDZH on a.DLYX=DDZH.B_DLYX where " + ViewState["ht_where"].ToString() + " order by a.LSCSSJ DESC,a.number DESC";
        ...
        MXC.goxls(NewDS, "Report_" + DateTime.Now.ToString("yyyy-MM-dd:hh-mm-sss") + ".xls", "违约赔偿发生记录", "违约赔偿发生记录", 15);
    }
}