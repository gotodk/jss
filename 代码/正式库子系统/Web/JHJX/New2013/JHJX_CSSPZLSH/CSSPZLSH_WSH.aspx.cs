...
public partial class Web_JHJX_New2013_JHJX_CSSPZLSH_CSSPZLSH_WSH : System.Web.UI.Page
{
    ...
    private Hashtable SetV()
    {
        ...
        ht_where["search_tbname"] = " ( select a.Number, a.DLYX 交易方账号,b.I_JYFMC 交易方名称,a.SQSJ 申请时间,a.SHZT 审核状态,a.SPBH 商品编号,c.SPMC 商品名称,a.SFBGZZHBZ 是否变更资质或标准,c.SFYX 是否有效 from AAA_CSSPB a left join AAA_DLZHXXB b on a.DLYX=b.B_DLYX left join AAA_PTSPXXB c on a.SPBH=c.SPBH where a.SHZT='未审核' ) as tab ";
        ht_where["search_str_where"] = " 1=1 ";  //检索条件
        ht_where["search_mainid"] = " 商品编号 ";  //所检索表的主键
        ht_where["search_paixu"] = " desc ";  //排序方式
        ht_where["search_paixuZD"] = " 申请时间";  //用于排序的字段
    ...
    private void MyWebControl_OnNeedLoadData(DataSet NewDS, string ERRinfo)
    {
        ...
        {
            DataSet ds = DbHelperSQL.Query("select '交易方账号'='','交易方名称'='','申请时间'='','商品编号'='','商品名称'='','审核状态'='','是否变更资质或标准'='' from AAA_CSSPB  where 1!=1");
    ...
    public void ToExcel(System.Web.UI.Control ctl)
    {
        ...
        string filename = "JYPT_CSSPZLSHWSH" + System.DateTime.Now.ToString("_yyyyMMddHHmm");
    ...
    protected void Repeater1_ItemCommand(object source, RepeaterCommandEventArgs e)
    {
        if (e.CommandArgument == "ck")
        {
            ...
            //Response.Redirect("JHJX_CKXQ.aspx?dlyx=" + str[0].ToString().Trim() + "&spbh=" + str[1].ToString().Trim() + "&GoBackUrl=CSSPZLSH_WSH.aspx");
        }
        else if(e.CommandArgument=="sh")
        {
            //string[] str = e.CommandName.Split('&');

            Response.Redirect("JHJX_CKXQ.aspx?number=" + e.CommandName.Trim() + "&GoBackUrl=CSSPZLSH_WSH.aspx&sh=ok");
        }
    }
    protected void btnToExcel_Click(object sender, EventArgs e)
    {
        string str = "select * from  ( select a.Number, a.DLYX 交易方账号,b.I_JYFMC 交易方名称,a.SQSJ 申请时间,a.SHZT 审核状态,a.SPBH 商品编号,c.SPMC 商品名称,a.SFBGZZHBZ 是否变更资质或标准,c.SFYX 是否有效 from AAA_CSSPB a left join AAA_DLZHXXB b on a.DLYX=b.B_DLYX left join AAA_PTSPXXB c on a.SPBH=c.SPBH  where a.SHZT='未审核') as tab ";
        str += " where 交易方账号 like '%" + txtJYFZH.Text.Trim() + "%' and 交易方名称 like '%" + txtJYFMC.Text.Trim() + "%'";
        string StartTime = txtBeginTime.Text.Trim();
        string EndTime = txtEndTime.Text.Trim();
        if (StartTime != "" && EndTime == "")
        {
