...
public partial class Web_JHJX_New2013_JHJX_SPMM_jhjx_spmmjb : System.Web.UI.Page
{
        ...
        if (NewDS != null && NewDS.Tables[0].Rows.Count > 0)
        {
            ...
            for (int i = 0; i < NewDS.Tables[0].Rows.Count; i++)
            {
                ...
                if (ds_lowprice != null && ds_lowprice.Tables[0].Rows.Count > 0)
                {
                    ...
                    {                        
                        NewDS.Tables[0].Rows[i]["最低价标的拟售量达成率"] = (Convert.ToDouble(ds_lowprice.Tables[0].Rows[0]["已中标数量"].ToString()) / Convert.ToDouble(ds_lowprice.Tables[0].Rows[0]["最低价投标拟售量"].ToString()) * 100.00).ToString("#0.00");
                    }
                    else
                    {
                        //获取最低价标的拟购买数量和
    ...
    private Hashtable SetV()
    {
       ...
       // Hashtable ht_where = new Hashtable();
        ...
        ht_where["serach_Row_str"] = "tab.*,c.I_JYFMC as 交易方名称,c.I_PTGLJG as 业务管理部门,c.I_YWGLBMFL as 业务管理部门分类,'---' as 当前卖家最低价,'---' as 最低价标的经济批量,'---' as 最低价标的拟售量达成率 ";
        ...
        ht_where["search_str_where"] = " isnull(ljq.SFJRLJQ,'否')='否' ";  //检索条件。即时的合同“是否进入冷静期”字段的值一定不是“是”，所以不需要再加条件了。
        ht_where["search_paixu"] = " DESC ";  //排序方式
        ht_where["search_paixuZD"] = " tab.下单时间 ";  //用于排序的字段
        return ht_where;
    }
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        if (this.rptSPXX != null && this.rptSPXX.Items.Count > 0)
        {
            ...
            string sql = "select c.I_PTGLJG as 业务管理部门,tab.交易方账号,c.I_JYFMC as 交易方名称,tab.下单时间,tab.商品编号,tab.商品名称,tab.规格,tab.合同期限,tab.数量,tab.价格,tab.金额,'---' as 当前卖家最低价,'---' as 最低价标的经济批量,'---' as [最低价标的达成率/中标率%],tab.单据类型,tab.单据编号,tab.是否拆单,tab.供货或收货区域,c.I_YWGLBMFL as 业务管理部门分类 from  AAA_View_SPJingBiao as tab left join AAA_LJQDQZTXXB  as ljq on ljq.SPBH=tab.商品编号 and ljq.HTQX=tab.合同期限 left join AAA_DLZHXXB as c on c.B_DLYX=tab.交易方账号 where " + hidwhere.Value.ToString() + " order by tab.下单时间 desc";
            ...
            MXC.goxls(dataSet, "Report_" + DateTime.Now.ToString("yyyy-MM-dd:hh-mm-sss") + ".xls", "商品买卖竞标中汇总表(竞标中)", "商品买卖竞标中汇总表(竞标中)", 15);
        }
        else
        {
