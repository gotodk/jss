...
public partial class Web_JHJX_New2013_JHJX_PTZHZLSH_JYPT_WSH_info : System.Web.UI.Page
{
    ...
    protected void btnPass_Click(object sender, EventArgs e)
    {
        ...
        this.btnJYDJ.Enabled = false;
        if (!string.IsNullOrEmpty(this.txt_FWZX_SHYJ.Text.Trim()))
        {
            if (this.txt_FWZX_SHYJ.Text.Trim().Contains("'"))
            {
                this.txt_FWZX_SHYJ.Text = this.txt_FWZX_SHYJ.Text.Trim().Replace("'", "‘");
            }
            if (this.txt_FWZX_SHYJ.Text.Trim().Length > 1000)
            {
     this.ClientScript.RegisterStartupScript(GetType(), "message", "<script>window.top.Dialog.WarningConfirm('您输入的审核意见字符过长，请限制在1000个字符以内！');</script>");
            ...
            DbHelperSQL.ExecuteSql("insert AAA_JYZHZSB(Number,DLYX,JYFBH,JSZHLX,GLJJRJSBH,PTGLJG,FWZXSHZT,FWZXSHYJ,FWZXSHSJ,FWZXSHR,JYGLBSHZT) values('" + strJYZHZSB + "','" + this.lbl_JYFZH.Text.Trim() + "','" + ViewState["JYFBH"].ToString() + "','" + this.lbl_JYZHLX.Text.Trim() + "','" + ViewState["GLJJRJSBH"].ToString() + "','" + ViewState["PTGLJG"].ToString() + "','审核通过','" + this.txt_FWZX_SHYJ.Text.Trim() + "','" + DateTime.Now.ToString() + "','" + objName + "','尚未处理')");
            string url = "New2013/JHJX_PTZHZLSH/JYPT_WSH.aspx";
    ...
    protected void btnJYDJ_Click(object sender, EventArgs e)
    {
        this.btnPass.Enabled = false;
        this.btnJYDJ.Enabled = false;
        if (!string.IsNullOrEmpty(this.txt_FWZX_SHYJ.Text.Trim()))
        {
            ...
            object objName = us_ls.Name.ToString();
            string url = "New2013/JHJX_PTZHZLSH/JYPT_WSH.aspx";
            DbHelperSQL.ExecuteSql("insert AAA_JYZHZSB(Number,DLYX,JYFBH,JSZHLX,GLJJRJSBH,PTGLJG,FWZXSHZT,FWZXSHYJ,FWZXSHSJ,FWZXSHR,JYGLBSHZT) values('" + strJYZHZSB + "','" + this.lbl_JYFZH.Text.Trim() + "','" + ViewState["JYFBH"].ToString() + "','" + this.lbl_JYZHLX.Text.Trim() + "','" + ViewState["GLJJRJSBH"].ToString() + "','" + ViewState["PTGLJG"].ToString() + "','建议冻结','" + this.txt_FWZX_SHYJ.Text.Trim() + "','" + DateTime.Now.ToString() + "','" + objName + "','尚未处理')");
            this.ClientScript.RegisterStartupScript(GetType(), "message", "<script>window.top.Dialog.alert('审核信息提交成功！', function () {  var a = window.location.href; var b = a.substring(0, a.indexOf('New2013/'));  var newurl = b+'" + url + "'; window.location.href=newurl;   });</script>");
        }
    ...
    {
        Response.Redirect("JYPT_WSH.aspx");
    }
}