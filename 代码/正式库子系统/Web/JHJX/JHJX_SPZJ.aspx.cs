...
public partial class Web_JHJX_JHJX_SPZJ : System.Web.UI.Page
{
    //用于无限极下拉框数据源
    DataView dv;
    //层次分隔符
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            string str = "INSERT INTO [AAA_PTSPXXB]([Number],[SPBH],[SPMC],[GG],[SSFLBH],[JJDW],[JJPL],[SPMS],[SCZZYQ],[FBRQ],[SFYX],[CheckState],[CreateUser],[CreateTime]) VALUES('" + KeyNumber + "','" + spbh + "','" + txtSPMC.Text.Trim() + "','" + txtSPGG.Text.Trim() + "'," + ddlSSFL.SelectedValue.Trim() + ",'" + txtJJDW.Text.Trim() + "','" + txtJJPL.Text.Trim() + "','" + txtSPMS.Text.Trim() + "','" + zzxq + "',GETDATE(),'是',1,'" + User.Identity.Name.ToString() + "',GETDATE())";
            ...
            if (list.Count > 0)
            {
                ...
                {
                    string url = "JHJX_SPZJ.aspx";
                    MessageBox.ShowAndRedirect(this, "商品添加成功！", url);

                }
                else
                {
                    MessageBox.Show(this, "提交失败，请核对信息后重新提交！");
                    return;
                }
            }
