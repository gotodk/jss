...
public class XYDJJFCL
{
    ...
    protected ArrayList GetSqlText(Hashtable htInput)
    {
        ...
        //先从redis中读取
        string num = htInput["对照表编号"].ToString().Trim();
        DataTable dt_DZB = XYDZBRedis.Redis_GetXYBDMXDZB(num);
        ...
        if (dt_DZB == null)
        {
            ...
            dt_DZB = ((DataSet)returnHT1["return_ds"]).Tables[0];

            if (dt_DZB != null && dt_DZB.Rows.Count > 0)
            { XYDZBRedis.Redis_SetXYBDMXDZB(num, dt_DZB); }
        }              
