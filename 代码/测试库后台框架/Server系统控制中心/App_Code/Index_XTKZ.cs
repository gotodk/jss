...
public class Index_XTKZ
{
            ...
            if (ds != null && ds.Tables[0].Rows.Count > 0)
            {
                ...
                for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                {
                    ...
                    if (ht["type"].ToString().Equals("集合经销平台"))
                    {
                        ...
                        //写入Redis，标记上该用户有新提醒了。
                        HomePageRedis.AddNewmessage(ht["提醒对象登陆邮箱"].ToString());
                    }
                    #endregion
                    #region//业务平台
    ...
    public static string GetNextNumberZZ_XTKZ_redis(string ModuleName, string HeadStr)
    {//2014-11-10 shiyan 从redis中获取模块编号
        try
        {
            if (string.IsNullOrEmpty(ModuleName))
            {
            ...
            string KeyNumber = PublicHelp.GetPrimKey(ModuleName);
            if (KeyNumber != null)
            {
                KeyNumber = HeadStr + KeyNumber;
            }
            return KeyNumber;
        }
        catch (Exception)
    ...
    public static DataSet CheckFormTrayMsg_XTKZ(string dlyx)
    {
        ...
        try
        {
            ...
            //检查是否要踢人
            bool T = HomePageRedis.CheckTiRen();
            if(T)
            {
                //可以登陆时"ok"   要踢人时"提醒内容"
            ...
            //检查是否需要去数据库找提醒
            bool MM = HomePageRedis.CheckNewmessage(dlyx);
            if (!MM)
            {
                dsreturn.Tables["返回值单条"].Rows[0]["执行结果"] = "err";
    ...
    public static DataSet GetParameterInfo_XTKZ()
    {
        try
        {
             ...
             */
            /***********wyh 2014.11.04 修改****************/
            DataTable dtpt = new RDisPtdtcs().Read_PTDTCS(null);
            if (dtpt != null && dtpt.Rows.Count > 0)
            {
                ds_p = new DataSet();
                ds_p.Tables.Add(dtpt);
            }
            else
            {
                Hashtable ht = I_DBL.RunProc("select top 1 * from AAA_PTDTCSSDB order by CreateTime DESC", "");
                ...
                ds_p = (DataSet)ht["return_ds"];
                new RDisPtdtcs().Updte_PTDTCSB(null);
            }
            
            if (ds_p != null && ds_p.Tables[0].Rows.Count > 0)
            {
                HTParameterInfo["卖家投标保证金比率"] = ds_p.Tables[0].Rows[0]["MJTBBZJBL"];
                HTParameterInfo["卖家投标保证金最小值"] = ds_p.Tables[0].Rows[0]["MJTBBZJZXZ"];
                HTParameterInfo["卖家履约保证金比率"] = ds_p.Tables[0].Rows[0]["MJLYBZJBL"];
                HTParameterInfo["卖家履约保证金冻结期限"] = ds_p.Tables[0].Rows[0]["MJLYBZJDJQX"];
                HTParameterInfo["履约保证金不足比率"] = ds_p.Tables[0].Rows[0]["LYBZJBZBL"];
                HTParameterInfo["买家订金比率"] = ds_p.Tables[0].Rows[0]["MJDJBL"];
                HTParameterInfo["保证函比率"] = ds_p.Tables[0].Rows[0]["BZHBL"];
                HTParameterInfo["签收技术服务费比率"] = ds_p.Tables[0].Rows[0]["QSJSFWFBL"];
                HTParameterInfo["保证函外技术服务费比率"] = ds_p.Tables[0].Rows[0]["BZHWJSFWFBL"];
                HTParameterInfo["经纪人收益卖家比率"] = ds_p.Tables[0].Rows[0]["JJRSYMJBLsel"];
                HTParameterInfo["经纪人收益买家比率"] = ds_p.Tables[0].Rows[0]["JJRSYMJBLbuy"];
                HTParameterInfo["休眠期限"] = ds_p.Tables[0].Rows[0]["XMQX"];
                ...
                /*------------shiyan 2014-11-04 注释 改为从redis中获取------------
                ...
                -------------------------------------------------------------------*/
                //调用redis获取，返回是否假期。
                HTParameterInfo["是否在服务时间内(假期)"] = IsHoliday.ReadHoliday(DateTime.Now.ToString("yyyy-MM-dd"))=="是"?"否":"是";
    ...
    public static string IsInServeTime_XTKZ()
    {
        try
        {
            ...
            /*****wyh 2014.11.04 add******/
            DataSet ds_p = null;
            DataTable dt = new RDisPtdtcs().Read_PTDTCS("PTMRKSFWSJ,PTMRJSFWSJ");
            ...
            {
                Hashtable ht = I_DBL.RunProc("select top 1 PTMRKSFWSJ,PTMRJSFWSJ from AAA_PTDTCSSDB order by CreateTime DESC", "");
                if (!(bool)ht["return_float"])
                {
                    return null;
                }
