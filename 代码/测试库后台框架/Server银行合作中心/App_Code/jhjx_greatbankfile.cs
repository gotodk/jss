...
public class jhjx_greatbankfile
{
    ...
    public DataSet GetSqlwhere(DataSet dsreturn, DataTable ht)
    {
        ...
        string sql_wher = "select Number from   AAA_PingAnQSZTJLB where SJCSSSSYH='浦发银行'  and QSZT in('未处理','已发送')";
        Hashtable ht_wher = I_DBL.RunProc(sql_wher, "data");
        DataSet dswher = new DataSet();
        if ((bool)ht_wher["return_float"])
        {
            dswher = (DataSet)ht_wher["return_ds"];
        }
        else
        {
            dswher = null;
    ...
    public DataSet Getcgkhjyzijefile(DataSet dsreturn, DataTable ht)
    {
        try
        {
            ...
            input["@khyh"] = ht.Rows[0]["开户银行"].ToString();
            input["@strwhere"] = ht.Rows[0]["where"].ToString();

            string StrSql = " select ROW_NUMBER () over (order by 证券资金账号)  as '顺序流水号', * from (  select '' as '银行类型', I_YHZH as '存管账号/银行结算账号', '60050000' as '证券公司编号', '0000' as '营业部编码','' as '证券资金账号类型',I_ZQZJZH as '证券资金账号', '0' as '币种' ,SUM(je) as 清算金额 ,'' as 'T+N天数','' as '备注','' as '处理标记','' as '证券买入汇总金额', '' as '证券卖出汇总金额' from  (select  I_YHZH , I_ZQZJZH , (case YSLX when '冻结'  then -isnull(JE,0.00) when '扣减' then -isnull(JE,0.00) when '解冻' then isnull(JE,0.00) else isnull(JE,0.00) end) as je  from AAA_ZKLSMXB as a left join AAA_DLZHXXB as b on ( a.JSBH = b.J_BUYJSBH or a.JSBH = b.J_JJRJSBH or a.JSBH = b.J_SELJSBH )   where SJLX = '实' and XM <> '转出资金' and XM<> '转入资金' and b.I_KHYH = @khyh and  a.Number in ('+@strwhere+') )  as c group by  I_ZQZJZH,I_YHZH ) as tab "; //按清算状态记录表查找数据 wyh 2014.05.15 add
            //DataSet dsget = DbHelperSQL.Query(StrSql);
            Hashtable ht_get = I_DBL.RunParam_SQL(StrSql, "data", input);
            DataSet ds_get = new DataSet();
            if ((bool)ht_get["return_float"])
            {
                ds_get = (DataSet)ht_get["return_ds"];
            }
            else
            {
                ds_get = null;
            ...
            {
                DataTable dt = ds_get.Tables[0].Copy();
                ...
                Hashtable ht_update = I_DBL.RunParam_SQL(strupdate, input);                
            }
            else
            {
    ...
    public DataSet Getcgkhzjmc(DataSet dsreturn, DataTable ht)
    {
        try
        {
            ...
            //string Strbakdnam = ht.Rows[0]["开户银行"].ToString();
            string StrSqlget = " select I_YHZH as '存管账号/银行结算账号',I_ZQZJZH as '证券资金账号','0' as '币种',isnull(B_ZHDQKYYE,0.00) as 证券资金余额,'' as 未回买入资金,'' as 未回卖出资金,'' as 冻结资金, '' as 日期 from AAA_DLZHXXB where I_KHYH = @khyh and I_DSFCGZT = '开通'  ";
            //DataSet dsget = DbHelperSQL.Query(StrSqlget);
            Hashtable ht_get = I_DBL.RunParam_SQL(StrSqlget, "data", input);
            DataSet ds_get = new DataSet();
            if ((bool)ht_get["return_float"])
            {
                ds_get = (DataSet)ht_get["return_ds"];
            ...
            if (ds_get != null && ds_get.Tables[0].Rows.Count > 0)
            {
                DataTable dtable = ds_get.Tables[0].Copy();
    ...
    public DataSet GetYZZZXX(DataSet dsreturn, DataTable ht)
    {
        try
        {
           ...
           // string strGetYZZZXX = " select (case LYYWLX when '银行出金'  then '1' when '银行入金' then '1' when '券商入金' then '0' when '券商出金' then '0' end) '发起方', (case LYYWLX when '银行出金'  then '02' when '银行入金' then '01' when '券商入金' then '01' when '券商出金' then '02' end) '交易码',(select I_YHZH from AAA_DLZHXXB where B_DLYX=DD.B_DLYX ) '银行结算账号',(select I_ZQZJZH from AAA_DLZHXXB where B_DLYX=DD.B_DLYX ) '证券资金账号',JE '发生金额','0' '币种',LYDH '银行流水',ZZ.BDCJRJLSH '证券流水',convert(varchar(10),LSCSSJ,112) '交易日期',substring(convert(varchar(20),LSCSSJ,120),11,10) '交易时间','6' '银行类型','60050000' '券商编号','5' '业务类型' from  AAA_ZKLSMXB as ZZ   inner join AAA_DLZHXXB as DD on ZZ.DLYX=DD.B_DLYX where LYYWLX in ('银行入金','银行出金','券商发起','券商入金','券商出金') and DD.I_KHYH='" + ht.Rows[0]["开户银行"].ToString() + "' and convert(varchar(10),LSCSSJ,120)='" + jssj + "' ";
            ...
            input["@strwhere"] = ht.Rows[0]["where"].ToString();
            input["@khyh"] = ht.Rows[0]["开户银行"].ToString();
            string strGetYZZZXX = " select (case LYYWLX when '银行出金'  then '1' when '银行入金' then '1' when '券商入金' then '0' when '券商出金' then '0' end) '发起方', (case LYYWLX when '银行出金'  then '02' when '银行入金' then '01' when '券商入金' then '01' when '券商出金' then '02' end) '交易码',(select I_YHZH from AAA_DLZHXXB where B_DLYX=DD.B_DLYX ) '银行结算账号',(select I_ZQZJZH from AAA_DLZHXXB where B_DLYX=DD.B_DLYX ) '证券资金账号',JE '发生金额','0' '币种',LYDH '银行流水',ZZ.BDCJRJLSH '证券流水',convert(varchar(10),LSCSSJ,112) '交易日期',substring(convert(varchar(20),LSCSSJ,120),11,10) '交易时间','6' '银行类型','60050000' '券商编号','5' '业务类型' from  AAA_ZKLSMXB as ZZ   inner join AAA_DLZHXXB as DD on ZZ.DLYX=DD.B_DLYX where LYYWLX in ('银行入金','银行出金','券商发起','券商入金','券商出金') and DD.I_KHYH=@khyh and ZZ.Number in ('+@strwhere+') ";
            //DataSet dsget = DbHelperSQL.Query(strGetYZZZXX);
            Hashtable ht_get = I_DBL.RunParam_SQL(strGetYZZZXX, "data", input);
            DataSet ds_get = new DataSet();
            if ((bool)ht_get["return_float"])
            {
                ds_get = (DataSet)ht_get["return_ds"];
    ...
    public DataSet WCQS(DataSet dsreturn, DataTable ht)
    {
        try
        {
            ...
            I_Dblink I_DBL = (new DBFactory()).DbLinkSqlMain("");
            Hashtable input = new Hashtable();
            input["@qsrq"] = ht.Rows[0]["清算日期"].ToString();
            input["@khyh"] = ht.Rows[0]["开户银行"].ToString();
            input["@strwhere"] = strwhere;

            string strSqlTBDLBSJ = "insert AAA_JGZHTBB(YHYX,QSBH,TBYE,TBRQ,BYZT,KHYH,YHZH) select B_DLYX '登录邮箱',isnull(I_ZQZJZH,'') '证券资金账号',B_ZHDQKYYE '当前账户可用余额',@qsrq as '同步日期','' as '备用状态',I_KHYH '开户银行',isnull(I_YHZH,'') '银行账号' from AAA_DLZHXXB  where I_KHYH=@khyh";
            //更新登陆表的【第三方存管可用余额】
            string strSqlGXDLB = "update AAA_DLZHXXB set B_DSFCGKYYE=B_ZHDQKYYE where I_KHYH=@khyh";
            ...
            Hashtable ht_res = I_DBL.RunParam_SQL(arrayList, input);
            if ((bool)ht_res["return_float"])
            {
                dsreturn.Tables["返回值单条"].Rows[0]["执行结果"] = "ok";
            ...
            {
                dsreturn.Tables["返回值单条"].Rows[0]["执行结果"] = "err";
                dsreturn.Tables["返回值单条"].Rows[0]["提示文本"] = "执行清算失败，错误提示：！" + ht_res["return_errmsg"].ToString();
            }
