...
public class BankPufa
{
    ...
    public DataSet chujin(DataSet dsParams, BankInit BI)
    {
        ...
        Hashtable ht_input = new Hashtable();
        Hashtable ht_return = new Hashtable();
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                    ...
                    re = IPC.Call("获取账户的相关信息", new object[] { JSBH });
                    if (re[0].ToString() == "ok")
                    {
                    ...
                    DataTable dtDLB = dsuserinfo.Tables["完整数据集"]; //JBXX["完整数据集"] as DataTable;
                    ...
                    {
                        ht_input.Add("@DLYX", dlyx.Trim());
                        string strJJRXX = "select b.I_JYFMC 交易方名称,b.I_PTGLJG 平台管理机构 from AAA_MJMJJYZHYJJRZHGLB a left join AAA_DLZHXXB b on a.GLJJRDLZH=b.B_DLYX where a.SFDQMRJJR='是' and a.DLYX=@DLYX";

                        DataSet dsJJRXX = null;//DbHelperSQL.Query(strJJRXX);
                        ht_return = I_DBL.RunParam_SQL(strJJRXX, "jg", ht_input);

                        if ((bool)(ht_return["return_float"])) //说明执行完成
                        {
                            //这里就可以用了。
                            dsJJRXX = (DataSet)(ht_return["return_ds"]);
                    ...
                    //资金变动明细对照表
                    string key = GetKey("AAA_ZKLSMXB"); ;// WFM.numberFormat.GetNextNumberZZ("");
                    if (key.Equals(""))
                    {
                        dsreturn.Tables["返回值单条"].Rows[0]["执行结果"] = "err";
                        dsreturn.Tables["返回值单条"].Rows[0]["提示文本"] = "获取账款流水明细表主键出错。";
                        return dsreturn;
                    }
                ...
                if (zzlb == "商转银")
                {
                    ...
                    DataTable JBXXAll = dsuserinfo.Tables["完整数据集"];//JBXX["完整数据集"] as DataTable;
                    ...
                    else
                    {
                        //1.基本验证通过开始，证券段数据操作
                            ...
                            //写入资金流水明细
                            DataRow rows = GetMoneydbzDS("1304000002").Tables["DZB"].Rows[0];//dsDZB.Tables[0].Select("Number='1304000002'");

                            //string xm = rows["XM"].ToString().Trim();
                            //string xz = rows["XZ"].ToString().Trim();
                            //string zy = rows["ZY"].ToString().Trim().Replace("[x1]", BankLSH);
                            //string sjlx = rows["SJLX"].ToString().Trim();
                            //string yslx = rows["YSLX"].ToString().Trim();
                            htall["@Number"] = key;
                            htall["@XZ"] = rows["XZ"].ToString().Trim();
                            htall["@ZY"] = rows["ZY"].ToString().Trim();
                            htall["@SJLX"] = rows["SJLX"].ToString().Trim();
                            htall["@YSLX"] = rows["YSLX"].ToString().Trim();
                            htall["@XM"] = rows["XM"].ToString().Trim();
                            htall["@dlyx"] = dlyx;
                            htall["@JSZHLX"] = JSZHLX;
                            htall["@JSBH"] = JSBH;
                            htall["@LYYWLX"] = "券商出金";
                            htall["@LYDH"] = BankLSH;
                            htall["@LSCSSJ"] = BDTime;
                            htall["@BDCJRJLSH"] = SelLSH;
                            htall["@YDje"] = YDje;
                            htall["@JKBH"] = "";
                            htall["@CheckState"] = 0;

                            string str = "INSERT INTO [AAA_ZKLSMXB] ([Number],[DLYX],[JSZHLX],[JSBH],[LYYWLX],[LYDH],[LSCSSJ],[YSLX],[JE],[XM] ,[XZ],[ZY] ,[JKBH],[SJLX],BDCJRJLSH,[CheckState],[CreateUser],[CreateTime]) VALUES (@Number,@dlyx,@JSZHLX,@JSBH,@LYYWLX,@LYDH,@LSCSSJ,@YSLX,@YDje,@XM,@XZ,@ZY,@JKBH,@SJLX,@BDCJRJLSH,@CheckState,@dlyx,@LSCSSJ)";
                            list.Add(str);
                            //更改登录表可用余额
                            ...
                            #endregion
                            int i = 0;
                            ht_return = I_DBL.RunParam_SQL(list,htall);
                            if ((bool)(ht_return["return_float"])) // 
                            {
                                i=(int)ht_return["return_other"];
                            }
                            ...
                            if (i > 0)
                            {
                                ...
                                {
                                    DataTable dtSToBank;
                                    #region//zhouli 2014.08.26  作废 -- 正式库运行再调用此银行接口
                                    ...
                                    //#endregion
                                    #endregion
                                    #region//zhouli 2014.08.26  add -- 模拟运行期间生成固定DataSet，代替调用银行接口
                                    string strpass = "pass";
                                    dtSToBank = chujin_MNYX().Tables["result"];
                                    #endregion
                                        //判断银行操作是否成功
                                    ...
                                    else
                                    {
                                        try
                                        {
                                            ...
                                            string Keyyc =GetKey("AAA_CRJYCSJB"); 
                                            htall["@Keyyc"] = Keyyc;
                                            //在出入金异常信息表插入数据 
                                            string Strinsert = "INSERT INTO [AAA_CRJYCSJB] ([Number],[YSBH],[DLYX],[JSZHLX],[JSBH],[LYYWLX],[LYDH],[LSCSSJ],[YSLX],[JE],[XM] ,[XZ],[ZY] ,[JKBH],[SJLX],BDCRJLSH,[CheckState],[CreateUser],[CreateTime],ZT) VALUES (@Keyyc,@Number,@dlyx,@JSZHLX,@JSBH,@LYYWLX,@LYDH,@LSCSSJ,@YSLX,@YDje,@XM,@XZ,@ZY,@JKBH,@SJLX,@BDCJRJLSH,@CheckState,@dlyx,@LSCSSJ,'浦发出金异常')";
                                            al.Add(Strinsert);


                                            //从账款流水明细表删除信息
                                            string Strdele = " delete from  AAA_ZKLSMXB where Number=@Number";
    ...
    /// <summary>
    /// zhouli add 2014.08.26
    /// </summary>
    /// <returns></returns>
    public DataSet chujin_MNYX()
    {
        DataSet ds = new DataSet();
        DataTable dt = new DataTable();
        dt.TableName = "result";
        dt.Columns.Add("ErrorNo");
        dt.Columns.Add("bSerial");
        DataRow row = dt.NewRow();
        dt.Rows.Add(new object[] { "0000", "123123" });
        ds.Tables.Add(dt);
        return ds;
    }
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                if (CardType.ToString().Trim() == "1")
                {
                    strMM = "SELECT * FROM AAA_DLZHXXB WHERE B_JSZHMM='" + ZQZJMM + "' and I_YHZH='" + YHZH + "' and I_ZQZJZH='" + ZQZH + "' AND I_SFZH='" + Card + "'";
                     ...
                     ht_return = I_DBL.RunProc(sql, "ff");
                     dspp = (DataSet)ht_return["return_ds"];//DbHelperSQL.Query(sql);
                     ...
                     ht_return = I_DBL.RunProc(strMM, "ff");
                     dsmm = (DataSet)ht_return["return_ds"];//DbHelperSQL.Query(strMM);
                ...
                else if (CardType.ToString().Trim() == "9")
                {
                    ...
                    //验证账号与身份证是否匹配,由于在ws2013已经验证账号是否有效，此处不再验证
                    ht_return = I_DBL.RunProc(sql, "ff");
                    dspp = (DataSet)ht_return["return_ds"];
                    if (dspp == null || dspp.Tables[0].Rows.Count <= 0) //不匹配
                    {
                        dsreturn.Tables["返回值单条"].Rows[0]["执行结果"] = "err";
                        dsreturn.Tables["返回值单条"].Rows[0]["提示文本"] = "身份证号码不符";
                    ...
                    ht_return = I_DBL.RunProc(strMM, "ff");
                    dsmm = (DataSet)ht_return["return_ds"];
                    if (dsmm == null || dsmm.Tables[0].Rows.Count < 1) //身份证号与账号匹配，但是密码不匹配
                    {
                        dsreturn.Tables["返回值单条"].Rows[0]["执行结果"] = "err";
                        dsreturn.Tables["返回值单条"].Rows[0]["提示文本"] = "证券资金密码错误";
                ...
                double DSFCGKYYE = Convert.ToDouble(dsmm.Tables[0].Rows[0]["B_DSFCGKYYE"].ToString());//第三方存管可用余额
                       ...
                       // DataSet dsDZB = DbHelperSQL.Query("select * from AAA_moneyDZB where Number='1304000002'");
                        ...
                        if (KZYE >= YDje)
                        {
                            ...
                            int i = 0;//DbHelperSQL.ExecuteSqlTran(list);
                            ht_return = I_DBL.RunParam_SQL(list);
                            if ((bool)(ht_return["return_float"])) // 
                            {
                                i = (int)ht_return["return_other"];
                            }
        ...
        Hashtable ht_return = new Hashtable();
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                ...
                if (JSZHLX.Contains("买家卖家交易账户"))
                {
                    ...
                    ht_return = I_DBL.RunProc(strJJRXX, "strJJRXX");
                    DataSet dsJJRXX = (DataSet)ht_return["return_ds"];// DbHelperSQL.Query(strJJRXX);
                    GLJJR = dsJJRXX.Tables[0].Rows[0]["交易方名称"].ToString().Trim();//关联经纪人
                    PTGLJG = dsJJRXX.Tables[0].Rows[0]["平台管理机构"].ToString().Trim();//平台管理机构
                }
                else
                ...
                if (zzlb == "银转商")   //入金（先向银行发送包，银行处理完成后，再处理证券段数据）
                {
                    ...
                    #endregion
                    #region//zhouli 2014.08.26  add -- 模拟运行期间生成固定DataSet，代替调用银行接口
                    dt1 = rujin_MNYX(je).Tables["result"];
                    #endregion
                    ...
                    if (dt1.Rows[0]["ErrorNo"].ToString() == "0000")//交易成功
                    {
                        try
                        {
                            ...
                            #region//
                            //写入资金流水明细
                            DataRow rows = GetMoneydbzDS("1304000001").Tables["DZB"].Rows[0];//dsDZB.Tables[0].Select("Number='1304000001'");
                            string xm = rows["XM"].ToString().Trim();
                            string xz = rows["XZ"].ToString().Trim();
                            string zy = rows["ZY"].ToString().Trim().Replace("[x1]", BankLSH);
                            string sjlx = rows["SJLX"].ToString().Trim();
                            ...
                            {
                                #region//zhouli 2014.08.26  作废 -- 正式库运行再调用此银行接口发起冲银行转证券操作（24703），模拟运行期间暂停
                                ...
                                //    string DJNumber = GetKey("AAA_JhjxUserdongjieremark");//JHJXUserDJ.numberFormat.GetNextNumberZZ("");
                                ...
                                //    int j = 0;
                                //    ht_return = I_DBL.RunParam_SQL(arylist);
                                //    if ((bool)(ht_return["return_float"])) // 
                                //    {
                                //        j = (int)ht_return["return_other"];
                                //    }
    ...
    /// <summary>
    /// zhouli 2014.08.26 add
    /// </summary>
    /// <param name="je"></param>
    /// <returns></returns>
    public DataSet rujin_MNYX(double je)
    {
        DataSet ds = new DataSet();
        DataTable dt = new DataTable();
        dt.TableName = "result";
        dt.Columns.Add("ErrorNo");
        dt.Columns.Add("bSerial");
        dt.Columns.Add("ExSerial");
        dt.Columns.Add("OccurBala");
        DataRow row = dt.NewRow();
        dt.Rows.Add(new object[] { "0000", "123123", "111111", je });
        ds.Tables.Add(dt);
        return ds;
    }
        ...
        try
        {
            if (dt.Rows.Count > 0)
            {
                ...
                if (CardType.ToString().Trim() == "1")
                {
                    ...
                    ht_return = I_DBL.RunProc(strMM, "ff");
                    DataSet dsmm = (DataSet)ht_return["return_ds"];

                    ht_return = I_DBL.RunProc(sql, "ff");
                    DataSet dspp = (DataSet)ht_return["return_ds"];



                    #region//验证证券资金密码
                    if (dsmm.Tables[0].Rows.Count == 0 && dspp.Tables[0].Rows.Count > 0)
                    {
                        dsreturn.Tables["返回值单条"].Rows[0]["执行结果"] = "err";
                        dsreturn.Tables["返回值单条"].Rows[0]["提示文本"] = "证券资金密码错误";
                        dsreturn.Tables["返回值单条"].Rows[0]["附件信息1"] = "2001";
                ...
                ht_return = I_DBL.RunProc(sql, "ds");

                DataTable dtUInfo = ((DataSet)ht_return["return_ds"]).Tables[0]; //DbHelperSQL.Query(sql).Tables[0];
    ...
    public DataSet biangengzhanghu(DataSet dsParams, BankInit BI)
    {
        ...
        try
        {
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                ...
                DataRow JCYZ = dsParams.Tables[0].Rows[0];
              ...
              //  DataTable dtDLB = JBXX["完整数据集"] as DataTable;
                ...
                DataRow JBXX = dsuserinfo.Tables[0].Rows[0]; //PublicClass2013.GetUserInfo(JSBH);
                DataTable dtDLB = dsuserinfo.Tables["完整数据集"];//JBXX["完整数据集"] as DataTable;
                ...
                if (dtDLB != null)
                {
                    if (dtDLB.Rows.Count == 1)
                    {
                        ...
                        dtBank.Rows[0]["ExSerial"] = BankLSH;//外部流水号
                        dtBank.Rows[0]["bCustAcct"] = OldYHZH;//银行账号                    
                        dtBank.Rows[0]["sCustAcct"] = ZQZJZH;//证券资金账号
                        dtBank.Rows[0]["bNewAcct"] = NewYHZH;//新银行账号
                        dtBank.Rows[0]["Date"] = time;//交易日期
                        DataSet ds = Go(dtBank);//BankServicePF.N26710_ModifyBankCard_Push(dtBankYE);
                        DataTable dt1 = null;
                        if (ds.Tables["str"].Rows[0]["状态信息"].ToString().Equals("ok"))
                        {
                             dt1 = ds.Tables["result"];
                        ...
                        if (dt1.Rows[0]["ErrorNo"].ToString() == "0000")
                        {
                            ...
                            int i = 0;//DbHelperSQL.ExecuteSql(strUpDLB);
                            ht_return = I_DBL.RunProc(strUpDLB);
                            if ((bool)(ht_return["return_float"])) // 
                            {
                                i = (int)ht_return["return_other"];
                            }
    ...
    public DataSet yinhangduanhuanka(DataSet dt, BankInit BI)
    {
        ...
        try
        {
            ...
            if (dt != null && dt.Tables[0].Rows.Count > 0)
            {
                ...
                ht_return = I_DBL.RunProc(strDLB, "aa");
                DataSet dsDLB = (DataSet)ht_return["return_ds"];//DbHelperSQL.Query(strDLB);
                if (dsDLB != null)
                {
    ...
    public DataSet chongyinhangduanrujin(DataSet ds, BankInit BI)
    {
        ...
        try
        {
            if (dt.Rows.Count > 0)
            {
                ...
                DataSet dsZJLSMX = null;//DbHelperSQL.Query(str);
                ht_return = I_DBL.RunProc(str,"aa");
                dsZJLSMX = (DataSet)ht_return["return_ds"];
    ...
    public DataSet qianyueyuzhiding(DataSet dsParams, BankInit BI)
    {
        ...
        DataSet dsReturn = initReturnDataSet();//ws.initReturnDataSet();
        dsReturn.Tables["返回值单条"].Rows.Add(new string[] { "err", "初始化" });
        ...
        try
        {
            if (dtUserInfo.Rows.Count > 0)
            {
                ...
                if (CardType == "1")
                {
                    ...
                    ht_return = I_DBL.RunProc(sql);
                    DataSet dspp = (DataSet)ht_return["return_ds"];//DbHelperSQL.Query(sql);
                    if (dspp == null || dspp.Tables[0].Rows.Count <= 0) //不匹配
                    {
                        dsreturn.Tables["返回值单条"].Rows[0]["执行结果"] = "err";
                        dsreturn.Tables["返回值单条"].Rows[0]["提示文本"] = "身份证号码不符";
                ...
                //验证各机构审核是否通过
                ht_return = I_DBL.RunProc(sql,"aa");
                DataTable dtUInPT = ((DataSet)ht_return["return_ds"]).Tables[0]; //DbHelperSQL.Query(sql).Tables[0];
                if (dtUInPT.Rows.Count == 1)
                {
                    ...
                    ht_return = I_DBL.RunProc(sqlUpdate);
                    int a = (int)ht_return["return_other"];
    ...
    /// 获取主键 wyh 2014.07.18
    /// </summary>
    /// <param name="MudulName"></param>
    /// <returns></returns>
    public string GetKey( string MudulName )
    {
        string key = "";
        object[] re = IPC.Call("获取主键", new object[] { MudulName, "" });//--------------------
        if (re[0].ToString() == "ok")
        {
