...
public class anaGetPdata
{
    ...
    public DataSet GetCqZJZZ(DataSet ds_page,Hashtable ht_tiaojian)
    {
        try
        {
            ...
            #region//如果是第一页，则读取redis数据--zhouli add 2014.11.26
            if (ds_page.Tables[0].Rows[0]["page_index"].ToString()=="0")
            {
                ds_res = CRedisData.RedisZJZZ_HGetData(ht_tiaojian["用户邮箱"].ToString(), ht_tiaojian["模块编号"].ToString());
                if (ds_res!=null)
                {
                    return ds_res;
                }
            ...
            #region//如果是第一页，则更新redis数据--zhouli add 2014.11.26
            if (ds_page.Tables[0].Rows[0]["page_index"].ToString() == "0")
            {
                CRedisData.RedisZJZZ_HSetData(ht_tiaojian["用户邮箱"].ToString(), ht_tiaojian["模块编号"].ToString(), ds_res);
            }
            #endregion
            return ds_res;
        }
    ...
    public DataSet GetCqSPMMGK(DataSet ds_page, Hashtable ht_tiaojian)
    {
        try
        {
            ...
            //先判断是否从redis中读取数据
            bool fromRedis = ht_tiaojian["商品名称"].ToString().Trim() == "" && ht_tiaojian["单据类型"].ToString() == "请选择单据类别" && ds_page.Tables[0].Rows[0]["page_index"].ToString() == "0";
            if (fromRedis)
            {
                string key = "H:Cqu:" + ht_tiaojian["用户邮箱"].ToString().Trim();
                DataSet rds = CRedisData.Redis_HGetData(key, "商品买卖概况");

                if (rds == null)
                {
                    //直接调用本中心的“分页数据获取”方法 
                    ds_old = pd.GetPagerDB(ds_page);
                    if (ds_old != null)
                    { CRedisData.Redis_HSetData(key, "商品买卖概况", ds_old); }
    ...
    public DataSet GetCqJBZ(DataSet ds_page, Hashtable ht_tiaojian)
    {
        try
        {
            ...
            {
                string key = "H:Cqu:" + ht_tiaojian["用户邮箱"].ToString().Trim();
                DataSet rds = CRedisData.Redis_HGetData(key, "竞标中");
                ...
                if (rds == null)
                {
                    ...
                    ds_old = pd.GetPagerDB(ds_page);
                    if (ds_old != null)
                    { CRedisData.Redis_HSetData(key, "竞标中", ds_old); }
            ...
            {//如果返回值中的表0为主要数据，则对其中的部分字段进行二次处理
                ...
                ds_res = apr.JingBiaoRedis(ds_old);
            }
            else
            {//如果有表，表0不是主要数据，则不做处理直接返回
                ds_res = ds_old.Copy();
    ...
    public DataSet GetCqZB(DataSet ds_page, Hashtable ht_tiaojian)
    {
        try
        {
            ...
            {
                string key = "H:Cqu:" + ht_tiaojian["用户邮箱"].ToString().Trim();
                DataSet rds = CRedisData.Redis_HGetData(key, "中标");
                ...
                if (rds == null)
                {
                    ...
                    ds_res = pd.GetPagerDB(ds_page);
                    if (ds_res != null)
                    { CRedisData.Redis_HSetData(key, "中标", ds_res); }
    ...
    public DataSet GetCqDBYBZH(DataSet ds_page, Hashtable ht_tiaojian)
    {
        try
        {
            ...
            {
                string key = "H:Cqu:" + ht_tiaojian["用户邮箱"].ToString().Trim();
                DataSet rds = CRedisData.Redis_HGetData(key, "定标与保证函");
                ...
                if (rds == null)
                {
                    ...
                    ds_old = pd.GetPagerDB(ds_page);
                    if (ds_old != null)
                    { CRedisData.Redis_HSetData(key, "定标与保证函", ds_old); }
    ...
    public DataSet GetCqFB(DataSet ds_page, Hashtable ht_tiaojian)
    {
        try
        {
            ...
            {
                string key = "H:Cqu:" + ht_tiaojian["用户邮箱"].ToString().Trim();
                DataSet rds = CRedisData.Redis_HGetData(key, "废标");
                ...
                if (rds == null)
                {
                    ...
                    ds_old = pd.GetPagerDB(ds_page);
                    if (ds_old != null)
                    { CRedisData.Redis_HSetData(key, "废标", ds_old); }
    ...
    public DataSet GetCqQP(DataSet ds_page, Hashtable ht_tiaojian)
    {
        try
        {
            ...
            //先判断是否从redis中读取数据
            bool fromRedis = (ht_tiaojian["合同编号"].ToString().Trim() == "" || ht_tiaojian["合同编号"].ToString().Trim() == "电子购货合同编号") && ht_tiaojian["清盘状态"].ToString() == "请选择清盘状态" && ht_tiaojian["清盘类型"].ToString() == "请选择清盘类型" && ds_page.Tables[0].Rows[0]["page_index"].ToString() == "0";
            if (fromRedis)
            {
                string key = "H:Cqu:" + ht_tiaojian["用户邮箱"].ToString().Trim();
                DataSet rds = CRedisData.Redis_HGetData(key, "清盘");
                ...
                if (rds == null)
                {
                    ...
                    ds_res = pd.GetPagerDB(ds_page);
                    if (ds_res != null)
                    { CRedisData.Redis_HSetData(key, "清盘", ds_res); }
   ...
   /// <returns></returns>
    ...
    public DataSet GetCqHWSFGK(DataSet ds_page, Hashtable ht_tiaojian)
    {
        try
        {
            ...
            #region//如果是第一页，则读取redis数据--zhouli add 2014.11.28
            if (ds_page.Tables[0].Rows[0]["page_index"].ToString() == "0")
            {
                ds_res = CRedisData.RedisHWSF_HGetData(ht_tiaojian["买家编号"].ToString(), ht_tiaojian["标签名"].ToString());
                if (ds_res != null)
                {
                    return ds_res;
                }
            ...
            #region//如果是第一页，则更新redis数据--zhouli add 2014.11.28
            if (ds_page.Tables[0].Rows[0]["page_index"].ToString() == "0")
            {
                CRedisData.RedisHWSF_HSetData(ht_tiaojian["买家编号"].ToString(), ht_tiaojian["标签名"].ToString(), ds_res);
            }
            #endregion
    ...
    public DataSet GetCqYXDTHD(DataSet ds_page, Hashtable ht_tiaojian)
    {
        try
        {
            ...
            if (ds_page.Tables[0].Rows[0]["page_index"].ToString() == "0")
            {
                ds_res = CRedisData.Redis_HGetData(ht_tiaojian["用户邮箱"].ToString(), ht_tiaojian["标签名"].ToString());
                if (ds_res != null)
                {
                    return ds_res;
                }
            ...
            if (ds_page.Tables[0].Rows[0]["page_index"].ToString() == "0")
            {
                CRedisData.RedisZJZZ_HSetData(ht_tiaojian["用户邮箱"].ToString(), ht_tiaojian["标签名"].ToString(), ds_res);
            }
            #endregion
            return ds_res;
        }
