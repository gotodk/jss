...
public partial class mywork_JYPT_JYPT_SHJYJL_ZLBZ : System.Web.UI.Page
{
    ...
    {
        //平台现用名称
        object objXYMC = DbHelperSQL.GetSingle("select GG from AAA_PTSPXXB where spbh='" + hidSPBH.Value + "'");
        if (objXYMC != null && objXYMC.ToString() != "")
        {
            spanXYNR.InnerText = objXYMC.ToString();
        }

        string sql_jy = "select * from AAA_SPXXJYB where JYRDLYX='" + hidDLYX.Value + "' and spbh='"+hidSPBH .Value +"' and jysx='质量标准'";
        ...
        {
            string sql_zc = "select a.*,b.* from AAA_SPXXJYB_ZCXQ as a left join AAA_SPXXJYB as b on a.parentnumber=b.number where ZCRDLYX='" + hidDLYX.Value + "' and spbh='"+hidSPBH .Value +"' and jysx='质量标准'";
    ...
    {
        DataSet ds_jy = DbHelperSQL.Query("select * from AAA_SPXXJYB where jynr='" + txtSPXMC.Text.Trim() + "' and spbh='"+hidSPBH .Value +"' and jysx='质量标准'");
        if (ds_jy != null && ds_jy.Tables[0].Rows.Count > 0)
        {
            MessageBox.ShowAlertAndBack(this, "此质量标准已存在，您可以支持此标准或者提交新的质量标准！");
        }
        if (txtSPXMC.Text.Trim().Length > 100)
        {
            MessageBox.ShowAlertAndBack(this, "您的建议已经超过100字，请修改后重新提交！");
        }
        if (hidSPBH.Value != "" && hidDLYX.Value != "")
        {
            ...
            string sql_insert = "INSERT INTO [AAA_SPXXJYB]([Number],[JYRDLYX],[JYRYHM],[SPBH],[JYSX],[JYNR],[ZCCS],[CheckState],[CreateUser],[CreateTime],[CheckLimitTime]) VALUES('" + KeyNumber + "','" + hidDLYX.Value + "','" + hidYHM.Value + "','" + hidSPBH.Value + "','质量标准', '" + txtSPXMC.Text.Trim() + "',0,1,'admin','" + DateTime.Now.ToString() + "','" + DateTime.Now.ToString() + "')";
            try
            {
                DbHelperSQL.ExecuteSql(sql_insert);
                MessageBox.ShowAndRedirect(this, "质量标准提交成功！", "JYPT_SHJYJL_ZLBZ.aspx?spbh=" + hidSPBH.Value + "&dlyx=" + hidDLYX.Value);
            }
            catch
            {
                MessageBox.ShowAlertAndBack(this, "质量标准提交失败！");
            }
        }
    }
    protected void rpt_ItemCommand(object source, RepeaterCommandEventArgs e)
    {
        if (e.CommandName == "ZhiChi")
        {
            ...
            al.Add(sql_insert);
            string sql_update = "UPDATE AAA_SPXXJYB set ZCCS=ZCCS+1 where number='" + parentnumber + "' and SPBH='" + hidSPBH.Value + "' and jysx='质量标准'";
            al.Add(sql_update);
            ...
            try
            {
                ...
                MessageBox.ShowAndRedirect(this, "支持成功！", "JYPT_SHJYJL_ZLBZ.aspx?spbh=" + hidSPBH.Value + "&dlyx=" + hidDLYX.Value);
            }
            catch
            {
    ...
    private string[] AddJF(string parentnumber)
    {
        ...
        {
            //获取此条建议提供者的登录邮箱
            DataSet ds_tgz = DbHelperSQL.Query("select JYRDLYX,B_JSZHLX from AAA_SPXXJYB as a left join AAA_DLZHXXB as b on a.JYRDLYX=B_DLYX where a.number='" + parentnumber + "' and jysx='质量标准' and spbh='" + hidSPBH.Value + "'");
            string tgzyx = "";
            string zhlx = "";
