...
public partial class mywork_JYPT_JYPT_SHJYJL_YSBZ : System.Web.UI.Page
{
    ...
    private void SetContent()
    {
        ...
        string sql_jy = "select * from AAA_SPXXJYB where JYRDLYX='" + hidDLYX.Value + "' and spbh='"+hidSPBH .Value +"' and jysx='验收标准'";
        ...
        if (ds_jy != null && ds_jy.Tables[0].Rows.Count > 0)
        {
            ...
            //txtCK.Visible = true;
            //lblCK.Visible = false;
            divCK.InnerHtml = "您已提交了新的验收方法建议，内容如下：<br/>" + ds_jy.Tables[0].Rows[0]["JYNR"].ToString().Replace("\r\n", "<br/>");
            hidCan.Value = "false";
        }
        else
        {
            string sql_zc = "select a.*,b.* from AAA_SPXXJYB_ZCXQ as a left join AAA_SPXXJYB as b on a.parentnumber=b.number where ZCRDLYX='" + hidDLYX.Value + "' and spbh='"+hidSPBH .Value +"' and jysx='验收标准'";
            ...
            if (ds_zc != null && ds_zc.Tables[0].Rows.Count > 0)
            {
                ...
                //txtCK.Visible = false;
                //lblCK.Visible = true;
                divCK.InnerHtml = "您已经对用户 <span style=\"color :Red\">" + ds_zc.Tables[0].Rows[0]["JYRYHM"].ToString() + "</span> 提出的验收方法表示了支持。";
                hidCan.Value = "false";
            }
            else
            {
    ...
    {
        DataSet ds_jy = DbHelperSQL.Query("select * from AAA_SPXXJYB where jynr='" + txtSPXMC.Text.Trim() + "' and spbh='"+hidSPBH .Value  +"' and jysx='验收标准'");
        if (ds_jy != null && ds_jy.Tables[0].Rows.Count > 0)
        {
            MessageBox.ShowAlertAndBack(this, "此验收方法已存在，您可以支持此方法或者提交新的验收方法！");
        }
        if (txtSPXMC.Text.Trim().Length > 3000)
        {
            MessageBox.ShowAlertAndBack(this, "您的建议已经超过3000个字，请修改后重新提交！");
        }
        if (hidSPBH.Value != "" && hidDLYX.Value != "")
        {
            ...
            string sql_insert = "INSERT INTO [AAA_SPXXJYB]([Number],[JYRDLYX],[JYRYHM],[SPBH],[JYSX],[JYNR],[ZCCS],[CheckState],[CreateUser],[CreateTime],[CheckLimitTime]) VALUES('" + KeyNumber + "','" + hidDLYX.Value + "','" + hidYHM.Value + "','" + hidSPBH.Value + "','验收标准', '" + txtSPXMC.Text.Trim() + "',0,1,'admin','" + DateTime.Now.ToString() + "','" + DateTime.Now.ToString() + "')";
            try
            {
                DbHelperSQL.ExecuteSql(sql_insert);
                MessageBox.ShowAndRedirect(this, "验收方法提交成功！", "JYPT_SHJYJL_YSBZ.aspx?spbh=" + hidSPBH.Value + "&dlyx=" + hidDLYX.Value);
            }
            catch
            {
                MessageBox.ShowAlertAndBack(this, "验收方法提交失败！");
            }
        }
    }
    protected void rpt_ItemCommand(object source, RepeaterCommandEventArgs e)
    {
        if (e.CommandName == "ZhiChi")
        {
            ...
            al.Add(sql_insert);
            string sql_update = "UPDATE AAA_SPXXJYB set ZCCS=ZCCS+1 where number='" + parentnumber  + "' and  SPBH='" + hidSPBH.Value + "' and jysx='验收标准'";
            al.Add(sql_update);
            ...
            try
            {
                ...
                MessageBox.ShowAndRedirect(this, "支持成功！", "JYPT_SHJYJL_YSBZ.aspx?spbh=" + hidSPBH.Value + "&dlyx=" + hidDLYX.Value);
            }
            catch
            {
    ...
    private string[] AddJF(string parentnumber)
    {
        ...
        {
            //获取此条建议提供者的登录邮箱
            DataSet ds_tgz = DbHelperSQL.Query("select JYRDLYX,B_JSZHLX from AAA_SPXXJYB as a left join AAA_DLZHXXB as b on a.JYRDLYX=B_DLYX where a.number='" + parentnumber + "' and jysx='验收标准' and spbh='" + hidSPBH.Value + "'");
            string tgzyx = "";
            string zhlx = "";
