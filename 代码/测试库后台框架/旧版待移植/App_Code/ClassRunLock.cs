...
public class ClassRunLock
{
    ...
    public static string LockBegin_ByUser(string khbh)
    {
        ...
        string re = "0";
        object obj = DbHelperSQL.GetSingle("EXEC YWSSJMK_CheckLock '" + khbh + "'");
        if (obj != null)
        {
            ...
            if (obj.ToString() == "用户早已被锁定")
            {
                ...
                {
                    Thread.Sleep(5000);
                    object objr = DbHelperSQL.GetSingle("EXEC YWSSJMK_CheckLock '" + khbh + "'");
                    if (objr != null)
                    {
                ...
                //强制解锁
                DbHelperSQL.GetSingle("EXEC YWSSJMK_UnLock '" + khbh + "'");
                re = "2";
                return re;
