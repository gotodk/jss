...
public partial class pingtaiservices_moban_FMPTMJMJZHKTXY : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            if (Request.Params["Action"] != null && Request.Params["Action"].ToString() == "KT_ZQ")//账户开通之前查看协议
            {
                ...
                oldStr = BuildHtm(HttpContext.Current.Server.MapPath("/pingtaiservices/moban/FMPTMJMJZHKTXY.htm"));
                ...
                if (Request.Params["Number"] != null && Request.Params["Number"].ToString() != "")
                {
                    ...
                    //Response.End();
                     ...
                     if (re[0].ToString() == "ok")
                     {
                         ...
                         {
                             DataTable dataTable = ((DataSet)re[1]).Tables["用户信息"];
                             oldStr = BuildHtm(HttpContext.Current.Server.MapPath("/pingtaiservices/moban/FMPTMJMJZHKTXY.htm"));
                             newStr = oldStr.Replace("[YF]", dataTable.Rows[0]["交易方名称"].ToString()).Replace("[YYYY-MM-dd]", dataTable.Rows[0]["资料提交时间"].ToString() + "&nbsp;&nbsp;");
                             Response.Write(newStr);
                             Response.End();
