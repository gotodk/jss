...
public partial class pingtaiservices_moban_BZH : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        string oldstr = BuildHtm(HttpContext.Current.Server.MapPath("/pingtaiservices/moban/" + moban));
        ...
        DataSet ds = null;
        object[] re = IPC.Call("保证函获取中标定标表的信息", new object[] { Number.Trim() });
        if (re[0].ToString() == "ok")
        {
            if (re[1] != null && ((DataSet)re[1]).Tables.Count > 1 && ((DataSet)re[1]).Tables["主表"].Rows.Count > 0)
            {
                ds = (DataSet)re[1];
            }
            else
        ...
        if (ds != null && ds.Tables["主表"].Rows.Count > 0)
        {
            string newstr = "";
            //开始替换模板
            DateTime dataTime = Convert.ToDateTime(ds.Tables["主表"].Rows[0]["定标时间"]);

            newstr = oldstr.Replace("[保证函编号]", ds.Tables["主表"].Rows[0]["保证函编号"].ToString());//暂时没有保证函编号
            newstr = newstr.Replace("[卖家名称]", ds.Tables["主表"].Rows[0]["卖家名称"].ToString());
            newstr = newstr.Replace("[买家名称]", ds.Tables["主表"].Rows[0]["买家名称"].ToString());
            newstr = newstr.Replace("[保证函大写金额]", chang(ds.Tables["主表"].Rows[0]["保证函金额"].ToString()));
            newstr = newstr.Replace("[保证函小写金额]", ds.Tables["主表"].Rows[0]["保证函金额"].ToString());
            newstr = newstr.Replace("[电子购货合同编号]", ds.Tables["主表"].Rows[0]["合同编号"].ToString());
