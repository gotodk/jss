...
public partial class mywork_JYPT_JYPT_SHJYJL_YSBZ : System.Web.UI.Page
{
    ...
    private void SetContent()
    {
        ...
        string sql_jy = "select * from AAA_SPXXJYB where JYRDLYX=@DLYX and spbh=@spbh and jysx='验收标准'";
        ...
        {
            string sql_zc = "select a.*,b.* from AAA_SPXXJYB_ZCXQ as a left join AAA_SPXXJYB as b on a.parentnumber=b.number where ZCRDLYX=@DLYX and spbh=@spbh and jysx='验收标准'";
    ...
    { 
        ...
        input["@spbh"] = hidSPBH.Value.ToString();
        string sql_jy = "select * from AAA_SPXXJYB where jynr=@jynr and spbh=@spbh and jysx='验收标准'";

        Hashtable result_jy = I_DBL.RunParam_SQL(sql_jy, "验收", input);
        DataSet ds_jy = new DataSet();
        if ((bool)result_jy["return_float"])
        {
            ds_jy = (DataSet)result_jy["return_ds"];
        ...
        {
            Page.ClientScript.RegisterStartupScript(this.GetType(), "message", "<script>alert('您的建议已经超过3000个字，请修改后重新提交！');</script>");
            return;
        }
        ...
        if (hidSPBH.Value != "" && hidDLYX.Value != "")
        {
            ...
            string sql_insert = "INSERT INTO [AAA_SPXXJYB]([Number],[JYRDLYX],[JYRYHM],[SPBH],[JYSX],[JYNR],[ZCCS],[CheckState],[CreateUser],[CreateTime],[CheckLimitTime]) VALUES(@KeyNumber,@DLYX,@YHM,@spbh,'验收标准', @jynr,0,1,'admin',@CreateTime,@CreateTime)";
            ...
            if (((bool)result["return_float"]) == true && result["return_other"].ToString() == "1")
            {
                ...
                Page.ClientScript.RegisterStartupScript(GetType(), "message", "<script>alert('验收方法提交成功！');window.location.href='" + url + "';</script>");
            }
            else
            {
                Page.ClientScript.RegisterStartupScript(GetType(), "message", "<script>alert('验收方法提交失败！');</script>");
                return;
            }  
        }
    }
    protected void rpt_ItemCommand(object source, RepeaterCommandEventArgs e)
    {
        if (e.CommandName == "ZhiChi")
        {
            ...
            al.Add(sql_insert);
            string sql_update = "UPDATE AAA_SPXXJYB set ZCCS=ZCCS+1 where number=@parentnumber and  SPBH=@spbh and jysx='验收标准'";
            al.Add(sql_update);
    ...
    private void AddJF(I_Dblink I_DBL, string parentnumber, string jyzdlyx)
    {
        ...
        {//如果支持数量已经有5个了，则调用增加信用积分的方法
            ...
            dt.Columns.Add("登录邮箱");
            dt.Columns.Add("备注");
            dt.Rows.Add(new string[] { jyzdlyx, "验收方法" + parentnumber });
