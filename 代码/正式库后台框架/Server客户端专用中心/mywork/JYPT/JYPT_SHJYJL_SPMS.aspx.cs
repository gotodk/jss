...
{   
    ...
    {       
        ...
        //平台现用商品描述
        string sql_sel = "select SPMS from AAA_PTSPXXB where spbh=@spbh";       
        Hashtable result_XYMC = I_DBL.RunParam_SQL(sql_sel, "商品描述", input);
        if ((bool)result_XYMC["return_float"])
        { 
            DataSet dsxymc=(DataSet)result_XYMC["return_ds"];
            if (dsxymc != null && dsxymc.Tables[0].Rows.Count > 0)
            { 
                spanXYNR.InnerText =dsxymc.Tables [0].Rows [0]["SPMS"] .ToString();
            }
        }
       
        //确定是否提交过建议       
        string sql_jy = "select * from AAA_SPXXJYB where JYRDLYX=@DLYX and spbh=@spbh and jysx='商品描述'";
        Hashtable result_jy = I_DBL.RunParam_SQL(sql_jy, "提交描述", input);
        ...
        if (ds_jy != null && ds_jy.Tables["提交描述"].Rows.Count > 0)
        {
            trTJ.Visible = false;
            trCK.Visible = true;
        ...
        {//没有提交过建议的时候，再确定是否支持过某个建议           
            string sql_zc = "select a.*,b.* from AAA_SPXXJYB_ZCXQ as a left join AAA_SPXXJYB as b on a.parentnumber=b.number where ZCRDLYX=@DLYX and spbh=@spbh and jysx='商品描述'";
            DataSet ds_zc = new DataSet();
            Hashtable result_zc = I_DBL.RunParam_SQL(sql_zc, "支持描述", input);
    ...
    protected void btnTJ_Click(object sender, EventArgs e)
    {
        ...
        input["@spbh"] = hidSPBH.Value.ToString();
        string sql_jy = "select * from AAA_SPXXJYB where jynr=@jynr and spbh=@spbh and jysx='商品描述'";

        Hashtable result_jy = I_DBL.RunParam_SQL(sql_jy, "描述", input);
        DataSet ds_jy = new DataSet();
        if ((bool)result_jy["return_float"])
        {
            ds_jy = (DataSet)result_jy["return_ds"];
        ...
        {
            Page.ClientScript.RegisterStartupScript(this.GetType(), "message", "<script>alert('您的建议已经超过300个字，请修改后重新提交！');</script>");
            return;           
        }
        ...
        if (hidSPBH.Value != "" && hidDLYX.Value != "")
        {
            ...
            string sql_insert = "INSERT INTO [AAA_SPXXJYB]([Number],[JYRDLYX],[JYRYHM],[SPBH],[JYSX],[JYNR],[ZCCS],[CheckState],[CreateUser],[CreateTime],[CheckLimitTime]) VALUES(@KeyNumber,@DLYX,@YHM,@spbh,'商品描述', @jynr,0,1,'admin',@CreateTime,@CreateTime)";


            Hashtable result = I_DBL.RunParam_SQL(sql_insert, input);
            if (((bool)result["return_float"]) == true && result["return_other"].ToString() == "1")
            {
                string url = "JYPT_SHJYJL_SPMS.aspx?spbh=" + hidSPBH.Value + "&dlyx=" + hidDLYX.Value;
    ...
    {  
        ...
        if (e.CommandName == "ZhiChi")
        {
            ...
            al.Add(sql_insert);
            string sql_update = "UPDATE AAA_SPXXJYB set ZCCS=ZCCS+1 where Number=@parentNumber and jysx='商品描述' and spbh=@spbh";
            al.Add(sql_update);
    ...
    {  
        ...
        {//如果支持数量已经有5个了，则调用增加信用积分的方法
            ...
            dt.Columns.Add("登录邮箱");
            dt.Columns.Add("备注");
            dt.Rows.Add(new string[] { jyzdlyx,"商品描述"+parentnumber});
