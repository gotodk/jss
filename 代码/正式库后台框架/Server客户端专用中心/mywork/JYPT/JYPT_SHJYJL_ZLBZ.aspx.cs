...
public partial class mywork_JYPT_JYPT_SHJYJL_ZLBZ : System.Web.UI.Page
{
    ...
    private void SetContent()
    {
        ...
        //平台现用商品俗称
        string sql_sel = "select GG from AAA_PTSPXXB where spbh=@spbh";
        Hashtable result_XYMC = I_DBL.RunParam_SQL(sql_sel, "质量", input);
        if ((bool)result_XYMC["return_float"])
        {
            DataSet dsxymc = (DataSet)result_XYMC["return_ds"];
            if (dsxymc != null && dsxymc.Tables[0].Rows.Count > 0)
            {
                spanXYNR.InnerText = dsxymc.Tables[0].Rows[0]["GG"].ToString();
            }
        }
        else
        {
        ...
        string sql_jy = "select * from AAA_SPXXJYB where JYRDLYX=@DLYX and spbh=@spbh and jysx='质量标准'";
        ...
        if (ds_jy != null && ds_jy.Tables[0].Rows.Count > 0)
        {
            trTJ.Visible = false;
            trCK.Visible = true;
            lblCK.Text = "您已提交了新的质量标准建议，内容为：" + ds_jy.Tables[0].Rows[0]["JYNR"].ToString();
            hidCan.Value = "false";
        }
        else
        {
            string sql_zc = "select a.*,b.* from AAA_SPXXJYB_ZCXQ as a left join AAA_SPXXJYB as b on a.parentnumber=b.number where ZCRDLYX=@DLYX and spbh=@spbh and jysx='质量标准'";
            ...
            if (ds_zc != null && ds_zc.Tables[0].Rows.Count > 0)
            {
                trTJ.Visible = false;
                trCK.Visible = true;
                string cknr = "您已经对用户 <span style=\"color :Red\">" + ds_zc.Tables[0].Rows[0]["JYRYHM"].ToString() + "</span> 提出的质量标准表示了支持。";
                lblCK.Text = cknr;
                hidCan.Value = "false";
            }
            else
    ...
    private Hashtable SetV()
    {
        ...
        ht_where["search_tbname"] = " (select a.number,a.spbh,a.jynr as 建议内容,a.jyryhm as 建议者,a.JYRDLYX as 建议者邮箱,a.zccs+1 as 支持次数,参与总数,CONVERT(varchar(10), ROUND(CONVERT(float,(a.zccs+1))/CONVERT(float,参与总数)*100.00,2))+'%' as 支持率,ROUND(CONVERT(float,(a.zccs+1))/CONVERT(float,参与总数)*100.00,2) as zclsort from AAA_SPXXJYB as a left join (select spbh,jysx,SUM(ZCCS+1) as 参与总数 from AAA_SPXXJYB where jysx='质量标准' group by SPBH,jysx) as b on a.spbh=b.spbh and a.jysx=b.jysx where a.jysx='质量标准') as tab ";
    ...
    {       
        ...
        input["@spbh"] = hidSPBH.Value.ToString();
        string sql_jy = "select * from AAA_SPXXJYB where jynr=@jynr and spbh=@spbh and jysx='质量标准'";

        Hashtable result_jy = I_DBL.RunParam_SQL(sql_jy, "质量", input);
        DataSet ds_jy = new DataSet();
        if ((bool)result_jy["return_float"])
        {
            ds_jy = (DataSet)result_jy["return_ds"];
        ...
        if (ds_jy != null && ds_jy.Tables[0].Rows.Count > 0)
        {
            Page.ClientScript.RegisterStartupScript(this.GetType(), "message", "<script>alert('此质量标准已存在，您可以支持此标准或者提交新的质量标准！');</script>");
            return;
        }
        if (txtSPXMC.Text.Trim().Length > 100)
        {
            Page.ClientScript.RegisterStartupScript(this.GetType(), "message", "<script>alert('您的建议已经超过100字，请修改后重新提交！');</script>");
            return;
        }
        ...
        if (hidSPBH.Value != "" && hidDLYX.Value != "")
        {
            ...
            string sql_insert = "INSERT INTO [AAA_SPXXJYB]([Number],[JYRDLYX],[JYRYHM],[SPBH],[JYSX],[JYNR],[ZCCS],[CheckState],[CreateUser],[CreateTime],[CheckLimitTime]) VALUES(@KeyNumber,@DLYX,@YHM,@spbh,'质量标准', @jynr,0,1,'admin',@CreateTime,@CreateTime)";
            Hashtable result = I_DBL.RunParam_SQL(sql_insert, input);
            if (((bool)result["return_float"]) == true && result["return_other"].ToString() == "1")
            {
                string url = "JYPT_SHJYJL_ZLBZ.aspx?spbh=" + hidSPBH.Value + "&dlyx=" + hidDLYX.Value;
                Page.ClientScript.RegisterStartupScript(GetType(), "message", "<script>alert('质量标准提交成功！');window.location.href='" + url + "';</script>");
            }
            else
            {
                Page.ClientScript.RegisterStartupScript(GetType(), "message", "<script>alert('质量标准提交失败！');</script>");
                return;
            }
        }
    }
    protected void rpt_ItemCommand(object source, RepeaterCommandEventArgs e)
    {
        if (e.CommandName == "ZhiChi")
        {
            ...
            al.Add(sql_insert);
            string sql_update = "UPDATE AAA_SPXXJYB set ZCCS=ZCCS+1 where number=@parentnumber and SPBH=@spbh and jysx='质量标准'";
            al.Add(sql_update);
    ...
    private void AddJF(I_Dblink I_DBL, string parentnumber, string jyzdlyx)
    {
        ...
        {//如果支持数量已经有5个了，则调用增加信用积分的方法
            ...
            dt.Columns.Add("登录邮箱");
            dt.Columns.Add("备注");
            dt.Rows.Add(new string[] { jyzdlyx, "质量标准" + parentnumber });
