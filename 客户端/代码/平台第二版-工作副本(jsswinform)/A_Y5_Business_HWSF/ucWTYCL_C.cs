...
{
    public partial class ucWTYCL_C : UserControl
    {
             /// <summary>
        /// 查询条件
        /// </summary>
        ...
        private void dataGridView1_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (columnName == "问题类别及详情" && e.RowIndex > -1)
            {

                Hashtable ht = new Hashtable();
                ht["Number"] = dataGridView1.Rows[e.RowIndex].Cells["Number"].Value.ToString();
        ...
        private void ucWTYCL_C_Load(object sender, EventArgs e)
        {
            GetData();
        }
        ...
        private void btnExport_Click(object sender, EventArgs e)
        {
            ...
            //string[] HideColumns = new string[] { "Number","中标定标表编号" };
            ////设置导出语句
            //string sql = "select * from   (select a.Number, (case when ( a.F_DQZT  = '部分收货' or a.F_DQZT  = '已录入补发备注' or a.F_DQZT  = '补发货物无异议收货' ) then a.F_BUYBFSHCZSJ  when ( a.F_DQZT  = '有异议收货' or a.F_DQZT  = '有异议收货后无异议收货' or a.F_DQZT='卖家主动退货' ) then a.F_BUYYYYSHCZSJ  when ( a.F_DQZT  = '请重新发货' or a.F_DQZT  = '撤销' ) then a.F_BUYQZXFHCZSJ  end)  as 问题产生时间,case when (a.F_DQZT='部分收货' or a.F_DQZT='已录入补发备注' or a.F_DQZT='补发货物无异议收货' ) then '部分收货' when ( a.F_DQZT='有异议收货' or a.F_DQZT='有异议收货后无异议收货' or a.F_DQZT='卖家主动退货' ) then '有异议收货' when (a.F_DQZT='请重新发货' or a.F_DQZT='撤销'  ) then '请重新发货' end as 问题类别及详情 , (case when (a.F_DQZT = '补发货物无异议收货' or a.F_DQZT = '有异议收货后无异议收货' or a.F_DQZT = '卖家主动退货' or a.F_DQZT = '撤销' ) then '完成' else '未完成' end ) as 处理状态  ,'F'+a.Number as 发货单号,b.Z_SPBH as 商品编号,b.Z_SPMC as 商品名称,b.Z_GG as 规格,a.T_THSL as 发货数量,b.Z_ZBJG as 定标价格,a.T_DJHKJE as 发货金额,(select I_JYFMC from AAA_DLZHXXB where B_DLYX = b.Y_YSYDDDLYX) as 买方名称,(select I_JYFMC from AAA_DLZHXXB where B_DLYX = b.T_YSTBDDLYX) as 卖方名称 ,a.F_WLGSMC as 物流公司名称,a.F_WLDH as 物流单号,a.F_FPHM 发票编号,case a.F_FPSFSHTH when '是' then '发票随货同行' else '发票不随货同行'  end as '发票发送方式',b.Z_HTBH 合同编号,b.Number 中标定标表编号,case b.Z_QPZT when '未开始清盘' then '定标' else case b.Z_HTZT when '定标合同到期' then '合同期满'+b.Z_QPZT when '定标合同终止' then '废标'+b.Z_QPZT when '定标执行完成' then '合同完成'+b.Z_QPZT else '异常'   end   end as '合同状态'  from AAA_THDYFHDXXB as a left join AAA_ZBDBXXB as b on a.ZBDBXXBBH = b.Number  where F_DQZT in ('部分收货','已录入补发备注','补发货物无异议收货','有异议收货','有异议收货后无异议收货','卖家主动退货','请重新发货','撤销') and (b.Y_YSYDDMJJSBH = '" + PublicDS.PublisDsUser.Tables["用户信息"].Rows[0]["买家角色编号"].ToString() + "' or b.T_YSTBDMJJSBH ='" + PublicDS.PublisDsUser.Tables["用户信息"].Rows[0]["卖家角色编号"].ToString() + "') ) as tab  where  " + ht_where["search_str_where"].ToString() + " order by 问题产生时间 ";
            //cMyXls1.BeginRunFrom_ht_where(sql, HideColumns);

            Hashtable ht_export = new Hashtable();
            ht_export["filename"] = "问题与处理";
