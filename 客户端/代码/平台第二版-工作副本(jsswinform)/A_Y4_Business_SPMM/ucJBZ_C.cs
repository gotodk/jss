...
{
    public partial class ucJBZ_C : UserControl
    {
        /// <summary>
        /// 查询条件
        /// </summary>
        ...
        /// select top 1  MJSDJJPL from AAA_TBD  where TBJG=( select MIN(TBJG) from AAA_TBD  where  ZT= '竞标' and SPBH=a.SPBH and HTQX=a.HTQX )   and ZT= '竞标' order by CreateTime
        ///  select top 1  MJSDJJPL from AAA_TBD  where TBJG=( select MIN(TBJG) from AAA_TBD  where  ZT= '竞标' and SPBH=a.SPBH and HTQX=a.HTQX )   and ZT= '竞标' order by CreateTime
        ...
        private void setDefaultSearch()
        {
            ...
            //ht_where["serach_Row_str"] = " tab.*,'--' as 当前卖方最低价,'--' as 最低价标的经济批量,'--' as [最低价标的达成率/中标率] "; //检索字段(必须设置)
            ...
            //ht_where["search_str_where"] = " tab.交易方账号='"+dlyx+"' and isnull(ljq.SFJRLJQ,'否')='否' ";  //检索条件(必须设置)
            //ht_where["search_paixu"] = " desc ";  //排序方式(必须设置)
            //ht_where["search_paixuZD"] = " tab.下单时间 ";  //用于排序的字段(必须设置)
            //ht_where["method_retreatment"] = "SPMM_C|JingBiao"; //查询条件 
        ...
        private void ucJBZ_C_Load(object sender, EventArgs e)
        {
            //处理下拉框间距
        ...
        {           
            //string dlyx = PublicDS.PublisDsUser.Tables["用户信息"].Rows[0]["登录邮箱"].ToString().Trim();
            //string StrSql = "select tab.* from AAA_View_SPJingBiao as tab left join AAA_LJQDQZTXXB as ljq on ljq.SPBH=tab.商品编号 and ljq.HTQX=tab.合同期限 where " + ht_where["search_str_where"].ToString() + " order by tab.下单时间 desc";
            //string[] HideColumns = new string[] {"交易方账号" };
            //cMyXls1.BeginRunFrom_ht_where(StrSql, HideColumns);

            Hashtable ht_export = new Hashtable();
            ht_export["filename"] = "竞标中";
