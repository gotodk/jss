...
{
    public partial class ucFB_C : UserControl
    {
        /// <summary>
        /// 查询条件
        /// </summary>
        ...
        private void setDefaultSearch()
        {
            ...
            /*---2013-12-30 shiyan 数据获取方式修改---*/
            string dlyx = PublicDS.PublisDsUser.Tables["用户信息"].Rows[0]["登录邮箱"].ToString().Trim();          
            //ht_where["serach_Row_str"] = " *,'--' as 履约保证金扣罚金额 "; //检索字段(必须设置)
            //ht_where["search_tbname"] = " AAA_View_SPFeiBiao ";  //检索的表(必须设置)
            ...
            //ht_where["search_paixuZD"] = " 废标时间 ";  //用于排序的字段(必须设置)
            //ht_where["method_retreatment"] = "SPMM_C|FeiBiao";
        ...
        private void ucFB_C_Load(object sender, EventArgs e)
        {
            //处理下拉框间距
        ...
        {
            //string dlyx = PublicDS.PublisDsUser.Tables["用户信息"].Rows[0]["登录邮箱"].ToString().Trim();
            //string sql = "select tab.废标时间,tab.废标原因,tab.下单时间,tab.商品编号,tab.商品名称,tab.规格,tab.合同期限,tab.数量,tab.价格,tab.金额,tab.单据类型,tab.单据编号,tab.订金解冻金额,tab.投标保证金扣罚金额,(case 合同状态 when '未定标废标' then '--' else (case tab.单据类型 when '预订单' then '--' else isnull(cast(c.扣罚履约保证金 as varchar(20)),'--') end) end) as 履约保证金扣罚金额,tab.是否拆单 from AAA_View_SPFeiBiao as tab left join (select LYDH,'投标单' as 单据类型,cast(sum(JE) as numeric(18,2)) as 扣罚履约保证金 from AAA_ZKLSMXB where SJLX='实' and (XZ='超过最迟发货日后录入发货信息' or XZ='发货单生成后5日内未录入发票邮寄信息' or XZ='电子购货合同履约保证金不足废标扣罚' or XZ='电子购货合同期满卖方未录入发货信息扣罚') and LYYWLX='AAA_ZBDBXXB' group by LYDH) as c on c.LYDH=tab.中标定标编号 and c.单据类型=tab.单据类型 where " + ht_where["search_str_where"].ToString() + " order by 废标时间 desc";

            //string[] HideColumns = new string[] {};
            //cMyXls1.BeginRunFrom_ht_where(sql, HideColumns);

            Hashtable ht_export = new Hashtable();
            ht_export["filename"] = "废标";
