...
namespace 客户端主程序.Support
{
    ...
    public class ManageDgv
    {
        ...
        public void AddMergeColumns(int paramColIndex, int paramColCount, string paramText, int paramRowNum)
        {
            ...
            {
                MergeColumns[i] = new MergeColumnsInfo(paramText, 7, paramColIndex, myRight, paramRowNum);
            }
        }
        ...
        private void PaintColumnsMerge(object sender, DataGridViewCellPaintingEventArgs e)
        {
            ...
            if (MergeColumns.ContainsKey(e.ColumnIndex))  // 被合并的列
            {
                ...
                {
                    int left = e.CellBounds.Left, top = e.CellBounds.Top + 1,//sdfs
                        right = e.CellBounds.Right, bottom = e.CellBounds.Bottom - 1;
                    // 画边框
                    ...
                    SizeF sizeText = e.Graphics.MeasureString(MergeColumns[e.ColumnIndex].Text, e.CellStyle.Font);
                    int kong = 2;


                    if (right - left < (int)(sizeText.Width) + kong)
                    {
                        // 画底色
                        g.FillRectangle(new SolidBrush(e.CellStyle.BackColor), left, top, (int)(sizeText.Width) + kong, bottom - top);
                    }
                    else
                    {
                        // 画底色
 ...
 
                    {
                        ...
                        if (right - left < (int)(sizeText.Width) + kong)
                        {
                            g.DrawString(MergeColumns[e.ColumnIndex].Text, e.CellStyle.Font, Brushes.Black, new Rectangle(left, top, (int)(sizeText.Width) + kong, bottom - top), sf);
                        }
                        else
                        {
                            g.DrawString(MergeColumns[e.ColumnIndex].Text, e.CellStyle.Font, Brushes.Black, new Rectangle(left, top, right - left, bottom - top), sf);
                        }
