/*
 * 功  能:钩子类，在调用此类的时候先项目--右键--属性--调试--启用 Visual Studio
 * 宿主进程勾去掉，因为钩子属于不安全代码，如果不去掉勾，调试的时候会报错！！！
 * 窗体调用钩子：
 * UserActivityHook actHook = new UserActivityHook();
 * actHook.OnMouseActivity+=new MouseEventHandler(事件);
 * actHook.KeyDown+=new KeyEventHandler(事件);
 * actHook.KeyPress+=new KeyPressEventHandler(事件);
 * actHook.KeyUp+=new KeyEventHandler(事件);
 */
