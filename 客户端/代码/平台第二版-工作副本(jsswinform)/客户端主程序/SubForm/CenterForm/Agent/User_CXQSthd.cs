...
{
    public partial class User_CXQSthd : UserControl
    {
        /// <summary>
        /// 查询条件
        /// </summary>
        ...
        private void setDefaultSearch()
        {
            ...
            ht_where["serach_Row_str"] = " row_number() over(order by 提货单号) id,* "; //检索字段(必须设置)
            ht_where["search_tbname"] = "  ( select '登陆邮箱'=BUYDLYX,'用户名'=BUYYHM,'分类'='提货','提货单号'=Number,'预订单号'=YDDH,'商品编号'=SPBH,'商品名称'=SPMC,'型号规格'=GGXH,'供货周期'=TBLX,'定标价格'=DBJG,'提货数量'=THZSL,'提货金额'=THZJE,'提货时间'=THDXDSJ,'发货时间'=SELFHSJ,'买家签收时间'=BUYQSSJ,'物流公司'=CYWLGSMC,'物流单号'=WLDH,'发票号'=FPH,'最迟发货日'=ZCFHR,'状态'=THDZT from ZZ_THDXXB where GLDLRDLYX='" + JSNM + "' union select '登陆邮箱'=SELDLYX,'用户名'=SELYHM,'分类'='发货','提货单号'=Number,'预订单号'=YDDH,'商品编号'=SPBH,'商品名称'=SPMC,'型号规格'=GGXH,'供货周期'=TBLX,'定标价格'=DBJG,'提货数量'=THZSL,'提货金额'=THZJE,'提货时间'=THDXDSJ,'发货时间'=SELFHSJ,'买家签收时间'=BUYQSSJ,'物流公司'=CYWLGSMC,'物流单号'=WLDH,'发票号'=FPH,'最迟发货日'=ZCFHR,'状态'=THDZT from ZZ_THDXXB where SELDLRDLYX='" + JSNM + "' ) as tab ";//检索的表(必须设置)
        ...
        private void BBsearch_Click(object sender, EventArgs e)
        {
            ...
            if (htzq != "不限状态")
            {
                ht_where["search_str_where"] = ht_where["search_str_where"] + " and 状态='" + htzq + "' ";
            }

            if (key != "" && key != "提货单号、商品编号、商品名称")
            {
                ht_where["search_str_where"] = ht_where["search_str_where"] + " and (提货单号 like '%" + key + "%' or 商品编号 like '%" + key + "%' or 商品名称 like '%" + key + "%') ";
            }
            if (txtBuyEmailOrName.Text.Trim() != "" && txtBuyEmailOrName.Text.Trim() != "登陆邮箱、用户名")
            {
                ht_where["search_str_where"] += " and (登陆邮箱 like '%" + txtBuyEmailOrName.Text.Trim() + "%' or 用户名 like '%" + txtBuyEmailOrName.Text.Trim() + "%') ";
            }
            //执行查询
            GetData(ht_where);
        }
