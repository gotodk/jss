...
{
    public partial class User_SEL_CXfeibiao : UserControl
    {
        /// <summary>
        /// 查询条件
        /// </summary>
        ...
        private void setDefaultSearch()
        {
            ...
            ht_where["search_tbname"] = "  ( select SELDLYX,SELYHM,'投标信息主表编号'=B.SELTBXXZBH,'投标信息子表编号'=B.MJTBXXChildBH,'标的号'=B.SELTBXXZBH+'_'+B.MJTBXXChildBH,'商品编号'=B.SPBH,'商品名称'=B.SPMC,'型号规格'=B.GGXH,'计价单位'=B.JJDW,'供货周期'=B.HTQX, '中标集合预订量'=sum(B.ZBSL),'中标价格'=B.SELTBJG,'中标金额'=sum(B.ZBZJE),'中标买家数量'=count(B.BUYJSBH),'投标时间'=B.SELCYJBSJ,'中标时间'=B.ZBSJ,'废标时间'=B.FBSJ,'废标原因'='卖家未按时缴纳履约保证金' from ZZ_ZBDBXXB as B where B.ZBDBZT='未定标废标'  and B.SELJJRDLYX='" + JSNM + "'  group by B.SELTBXXZBH,B.MJTBXXChildBH,B.SPBH,B.SPMC,B.GGXH,B.JJDW,B.HTQX,B.SELTBJG,B.ZBSJ,B.FBSJ,B.SELCYJBSJ, SELDLYX,SELYHM ) as tab  ";  //检索的表(必须设置)
        ...
        private void BBsearch_Click(object sender, EventArgs e)
        {
            ...
            //更改搜索条件
            setDefaultSearch();
            ht_where["search_str_where"] = ht_where["search_str_where"] + " and 废标时间 > '" + begintime + "' and 废标时间 < '" + endtime + "' ";  //检索条件(必须设置)
            if (htzq != "不限周期")
            {
