...
{
    public partial class ucTZJL_B : UserControl
    {
        ...
        private void setDefaultSearch()
        {
            ...
            //ht_where["serach_Row_str"] = " Number, TXDXDLYX, TXSJ,LTRIM(TXNRWB) AS TXNRWB "; //检索字段(必须设置)
            //ht_where["search_tbname"] = "  AAA_PTTXXXB  ";  //检索的表(必须设置)
            //ht_where["search_mainid"] = " Number ";  //所检索表的主键(必须设置)
            //ht_where["search_str_where"] = " 1=1 and TXDXDLYX='"+dlyx+"'  ";  //检索条件(必须设置)
            //ht_where["search_paixu"] = " DESC ";  //排序方式(必须设置)
            //ht_where["search_paixuZD"] = " TXSJ ";  //用于排序的字段(必须设置)
            ...
            ht_tiaojian["标签名"] = "通知记录";
            ht_tiaojian["用户邮箱"] = dlyx;
            ht_tiaojian["通知内容"] = txtTZNR.Text.Trim();
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            //更改搜索条件
            setDefaultSearch();
            //ht_where["search_str_where"] += " and  TXNRWB like '%"+txtTZNR.Text.Trim()+"%' ";  //检索条件(必须设置)
            //执行查询
            GetData(ht_where);
        }
