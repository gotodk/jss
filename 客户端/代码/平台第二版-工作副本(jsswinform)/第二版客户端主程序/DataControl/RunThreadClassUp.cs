        ...
        public void BeginDownLoad()
        {
            ...
            for (int i = 0; i < htUpdateFile.Count; i++)
            {
                ...
                try
                {
                    ...
                    AutoUpdate.ThreadDownLoad TDL = new AutoUpdate.ThreadDownLoad(updateFileUrl, 60*10000*5, Path.GetDirectoryName(tempPath) + "\\", true, false, CC);
                    string s=  TDL.BeginDown();
                    if (s == "err")
                    {
                        OutPutHT["下载失败"] = "文件下载失败！";//
                        DForThread(OutPutHT);
                    }
