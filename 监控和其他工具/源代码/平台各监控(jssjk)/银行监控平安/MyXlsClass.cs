...
namespace 银行监控平安
{
    public class MyXlsClass
    {
        ...
        /// <summary>
        /// 将数据集中包含的所有datatable导出到同一个电子表格的的不同sheet中
        /// </summary>
        /// <param name="dsxls">数据集</param>
        /// <param name="filename">导出文件名</param>
        /// <param name="lujing">保存路径</param>
        public static void goxls(DataSet dsxls, string filename, string lujing)
        {
            int rowwidth = 15;//单元格宽度默认20;
            ...
            for (int i = 0; i < dsxls.Tables.Count; i++)
            {
                string sheetsname = dsxls.Tables[i].TableName;
                Worksheet sheet = doc.Workbook.Worksheets.Add(sheetsname);
                Cells cells = sheet.Cells;
                ColumnInfo colInfo = new ColumnInfo(doc, sheet);
                colInfo.ColumnIndexStart = 0;
