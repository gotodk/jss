    ...
    public class Core
    {
        ...
        public DataTable GetBaoti(byte[] b,int i)
        {
            ...
            if (strAry.Length - 1 != dt.Columns.Count)
                return null;
            //老的多行数据的代码去掉了，平安银行不是那种方式。
            //可以参见《平安交易资金监管系统介绍及接口补充说明(存管模式).doc》最下方的实例
        ...
        public DataTable Get1010(string Str1010)
        {
            //获取包体字符串
            //截取包体中不重复的字符串
            //获取剩余可能重复数据的数组
            //填充DataTable
            string[] ary = Str1010.Split(new string[] { "&" }, StringSplitOptions.None);
            List<string> list = new List<string>();
            //转成泛型，方便处理
            foreach (string str in ary)
                list.Add(str);
            //包体中，总记录数&起始记录号&是否结束包&子账户个数&  是不重复的，需要移除
            string[] aryNoLoop = new string[4];//前边不重复的四个字段
            //...
