...
namespace Galaxy.ClassLib.DataBaseFactory
{
	...
	/// </summary>
	public class Dblink_Access_Main : I_Dblink
	{
        ...
        /// Access数据库连接类构造函数
		/// </summary>
        public Dblink_Access_Main(string ConnConfig)
		{
        ...
        public string[] getregedit()
        {
            ...
            if (pregkey == null)
            {
                setregedit(@"user id=sa;password=100zzcom;initial catalog=sm;Server=192.168.0.64\SQL2005;Connect Timeout=30");
                pregkey = Registry.CurrentUser.OpenSubKey("Software\\小商品扫码购物", true);
            }
        ...
        private void Dispose(OleDbConnection Conn)
		{
			if(Conn!=null)
			{
				Conn.Close();
        ...
        Hashtable I_Dblink.UpdateMore(string oldtempcmd, string XMLpath)
        {
            try
            {
                ...
                Conn.Open();
                OleDbDataAdapter Da = new OleDbDataAdapter(oldtempcmd, Conn);
		...
		Hashtable I_Dblink.RunProc_CMD(string P_cmd,DataSet Ds)
		{
			try
			{
                ...
                comm.Connection = Conn;
                Conn.Open();
                OleDbDataAdapter Da = new OleDbDataAdapter(P_cmd, Conn);
